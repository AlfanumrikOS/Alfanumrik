...
| **DB-37** — `scripts/check-config-parity.sh` resolves both config paths from the wrong cwd, exits 1 with no message, invoked by nothing. Configs currently agree; 12 of 19 constants covered by no test (incl. `PROMPT_REV`, `MODEL_ROUTE_REV`) | P2 | — | **DONE 2026-08-31** — script DELETED, not repaired: `apps/host/src/__tests__/grounding/config-parity-values.test.ts` (REG-433) already compares every shared constant by VALUE, which the shell script never did (NAME-only). **Second, larger defect found while fixing this:** the script was chained `bash check-config-parity.sh && eslint …` inside `lint:ai-boundary`, so its unconditional `exit 1` meant the AI-boundary ESLint had **never run** — and `lint:ai-boundary` was in no workflow either. Three rules (`no-direct-ai-calls`, `no-direct-rag-rpc`, `no-canonical-write-outside-projector`) were at `error` enforcing nothing. Gate now blocking in CI's `quality` job via `scripts/check-ai-boundary.mjs` + `scripts/ai-boundary-baseline.json`. **Still open (ai-engineer):** both `grounding-config.ts` / `grounded-answer/config.ts` line-3 headers still name the deleted script. | read script + diff both configs | 0 CI invocations; 7 of 19 constants tested | value-parity test in CI; AI-boundary gate blocking with 8 recorded violations | `npx vitest run src/__tests__/grounding/` (81 passed); `node scripts/check-ai-boundary.mjs` exit 0 | — |
