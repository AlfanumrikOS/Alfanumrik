...
| `parent.child_erasure_requested` | ✅ Live | [`src/app/api/parent/children/[student_id]/request-erasure/route.ts`](../../src/app/api/parent/children/[student_id]/request-erasure/route.ts) POST. Phase D.3 (DPDP §15). Guardian-initiated 7-day grace-window erasure request. Payload omits the `reason` body — subscribers fetch it from `data_erasure_requests` if needed. `journey.ts` returns null (DPDP flow is not a learner-facing milestone). |
| `parent.child_erasure_cancelled` | ✅ Live | [`src/app/api/parent/children/[student_id]/request-erasure/route.ts`](../../src/app/api/parent/children/[student_id]/request-erasure/route.ts) DELETE. Phase D.3 (DPDP §15). Guardian cancelled before purge_at. Carries `elapsedSec` between requested and cancelled for analytics. `journey.ts` returns null. |
| `parent.child_erasure_completed` | ✅ Live (cron) | [`supabase/functions/data-erasure-purger/index.ts`](../../supabase/functions/data-erasure-purger/index.ts). Phase D.3 (DPDP §15). Emitted after the cron purger completes the cascade DELETE for an erasure request. `rowsDeleted` is a per-table map for ops dashboards. `journey.ts` returns null. |
