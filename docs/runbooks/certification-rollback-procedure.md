...
6. **Standalone accounts and the two teardown options.** `purge_certification_tenant` (and therefore the `teardown-certification-tenant.ts` wrapper) only tears down the school-scoped tenant — student, teacher, school_admin rows, the school itself, and its non-cascading child tables. The parent (guardian), super_admin, content_author, and support_staff accounts the seed script also creates are never school-scoped and are NOT covered by that RPC. There are now two ways to finish a full-run teardown:

   - **Preferred — one call for the whole run:** `purge_certification_run('<run_id_short>')` (migration `20260702190000_certification_run_teardown.sql`). This delegates the school-scoped part to `purge_certification_tenant` internally AND cleans the standalone guardian + admin_users accounts (clearing their 5 blocking inbound FKs first — 4 admin child tables deleted plus the `schools.paused_by_super_admin_id` audit pointer nulled) + the demo_accounts registry rows — in one guarded, idempotent, service-role-only call. `<run_id_short>` is the first 8 hex characters the seed script prints as `run_id_short:` (also derivable as the first 8 hex of the printed full `certification_run_id`). It is hard-scoped to the `@certification.alfanumrik.invalid` domain AND `is_demo = true`, refuses any `run_id_short` that is not exactly 8 lowercase hex chars, and returns a JSONB summary including `standalone_auth_user_ids` — the auth.users ids the caller should then delete via the GoTrue admin API (the RPC surfaces them but does not delete auth.users itself, matching every other purge function in this codebase). Example:
     ```sql
     SELECT purge_certification_run('a1b2c3d4');
     ```
   - **Legacy — RPC plus three manual DELETEs:** call `purge_certification_tenant('<school_id>')` for the tenant, then run the exact 3 `DELETE` statements the seed script prints at the end of a run (`printSummary()`'s "Standalone accounts NOT covered by purge_certification_tenant" section) via a service-role SQL session. Still valid; `purge_certification_run` simply folds all four steps into one accountable operation.

   Whichever option you use, include the standalone-account row counts in the Part 5 verification. NOTE: a run-scoped wrapper script around `purge_certification_run` (mirroring `teardown-certification-tenant.ts`) is an ops follow-up; until it exists, invoke `purge_certification_run` directly via a service-role SQL session (Supabase SQL editor or equivalent), after independently confirming the target project ref is NOT the production ref.
