# Alfanumrik — Google Play Store Listing

## App Name
Alfanumrik — Smart CBSE Learning

## Short Description (80 chars max)
AI-powered CBSE learning app for Class 6-12. Concepts, quizzes & AI tutor.

## Full Description (4000 chars max)
Alfanumrik is an AI-powered learning app built for Indian CBSE students from Class 6 to 12. Study smarter with structured concepts, practice quizzes, and Foxy — your personal AI tutor.

📚 STRUCTURED LEARNING
• Grade-specific subjects: Mathematics, Science, Physics, Chemistry, Biology, English, Hindi, Social Studies & more
• Chapter-wise concept pages with clear explanations
• Hindi translations for key terms
• Mark topics complete and track your progress

🦊 FOXY AI TUTOR
• Ask any doubt — get simple, student-friendly explanations
• Topic-scoped conversations for focused learning
• Quick prompts to get started instantly
• Works like chatting with the smartest friend in class

📝 PRACTICE QUIZZES
• Subject-wise MCQ quizzes with 10 questions each
• Timed quiz sessions with progress tracking
• Instant results with score breakdown
• Anti-cheat checks keep scores honest

📈 PERFORMANCE SCORE & FOXY COINS
• Your Performance Score reflects accuracy across recent quizzes
• Earn Foxy Coins for every correct answer, perfect quiz, and daily streak
• Spend coins on hint unlocks, theme packs, and Foxy avatar items
• Daily streak tracking to build study habits

📊 SMART DASHBOARD
• Daily usage overview — chats used, quizzes taken
• Performance Score trend + Foxy Coin balance at a glance
• Bloom's-level mastery breakdown per subject
• Quick access to all subjects
• Plan status and upgrade options

💎 FLEXIBLE PLANS
• Free plan: 5 Foxy chats + 5 quizzes per day
• Starter: 30 chats + 20 quizzes per day (monthly or yearly)
• Pro: 100 chats + unlimited quizzes per day (monthly or yearly)
• Unlimited: Unlimited everything (monthly or yearly)
• See in-app for current INR pricing — discounts available on yearly plans

🔒 SAFE & LIGHTWEIGHT
• No ads, no distractions
• Minimal app size, fast on budget phones
• Works well on slow networks
• Portrait-optimized for one-hand use

Built by Indian educators for Indian students. Alfanumrik makes CBSE learning simple, interactive, and fun.

Download now and start learning smarter! 🎯

## Category
Education

## Tags / Keywords
cbse, learning, study, class 10, class 12, science, maths, ai tutor, quiz, indian students, board exam, ncert, education, study app, exam preparation

## Content Rating Questionnaire Answers
- Violence: No
- Sexual content: No
- Language: No
- Controlled substances: No
- User interaction: Yes (AI chat — no user-to-user interaction)
- Location sharing: No
- Personal info collection: Yes (email for account)
- Ads: No
- Target audience: 13+ (students class 6-12)
→ Expected rating: Everyone / PEGI 3 / IARC 3+

## Contact Details (required)
- Email: alfanumrik10@gmail.com
- Website: https://alfanumrik.com (if available)
- Privacy Policy URL: (required — see privacy_policy.md)

## App Details
- Application ID: com.alfanumrik.app
- Default language: English (en-IN)
- Available on: Phone
- Free with in-app purchases (subscription)

---

# VISUAL ASSETS SPECIFICATIONS

## App Icon
- Size: 512 x 512 px (PNG, 32-bit, no transparency)
- Design: Letter "A" in warm brown (#8B6F47) on cream (#FBF8F4) background
  with rounded square shape and subtle shadow

## Feature Graphic (required)
- Size: 1024 x 500 px (JPG or PNG)
- Design suggestions:
  - Background: Warm cream gradient (#FBF8F4 → #F0E8D8)
  - Center: "Alfanumrik" logo + tagline "Smart CBSE Learning"
  - Left side: Phone mockup showing dashboard screen
  - Right side: 🦊 Foxy mascot illustration
  - Bottom: "Class 6-12 | AI Tutor | Quizzes | Free"

## Screenshots (minimum 2, recommended 5-8)
- Size: 1080 x 1920 px (9:16 portrait) or 1920 x 1080 (16:9 landscape)
- Recommended screenshots:
  1. Splash/Login screen — "Start Learning Today"
  2. Dashboard — "Track Your Progress"
  3. Subjects list — "All Your CBSE Subjects"
  4. Concept page — "Clear, Simple Explanations"
  5. Foxy chat — "Ask Foxy Anything"
  6. Quiz in progress — "Practice with Quizzes"
  7. Quiz results — "Earn XP & Level Up"
  8. Plans screen — "Affordable Plans for Everyone"

## Screenshot Frame Template
- Add a device frame (phone mockup) around each screenshot
- Add a short headline above/below each:
  - Use color: #8B6F47 (primary brown)
  - Font: Bold, 36-48px
  - Background: #FBF8F4 (cream)

---

# PRIVACY POLICY (outline)

You need a privacy policy URL. Here's what it should cover:

1. Data collected: Email, name, grade, usage data
2. Purpose: Account creation, personalized learning
3. Storage: Supabase (cloud), Hive (device cache)
4. Third parties: Razorpay (payments only)
5. Data retention: Until account deletion
6. Children's privacy: App is for 13+ students
7. Contact: alfanumrik10@gmail.com

Host this as a page on your website or as a GitHub Pages site.
