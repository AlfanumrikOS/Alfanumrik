...
export async function middleware(request: NextRequest) {
  ...
  // ── Layer 1.5: Tenant resolution (B2B multi-tenant) ──
  // Resolves hostname to school context, sets headers for downstream routes.
  const host = request.headers.get('host') || '';
  if (host && !isB2CDomain(host)) {
    try {
      ...
      if (school) {
        ...
        const headers = tenantHeadersFromContext(tenantCtx);
        for (const [key, value] of Object.entries(headers)) {
          response.headers.set(key, value);
        }
      }
    } catch {
      // Tenant resolution failure is non-blocking — B2C fallback
    }
  }
