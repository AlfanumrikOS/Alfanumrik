...
vi.mock('@/lib/usePermissions', () => ({ usePermissions: () => ({ can: () => false }) }));

// Flag + pulse hooks default OFF so the optional Pulse section never mounts.
vi.mock('@/lib/use-school-pulse-flag', () => ({ useSchoolPulseFlag: () => false }));
vi.mock('@/lib/use-school-provisioning', () => ({ useSchoolProvisioning: () => false }));
vi.mock('@/lib/pulse/use-pulse', () => ({ useSchoolPulse: () => ({ data: null, error: null, isLoading: false, mutate: vi.fn() }) }));
...
vi.mock('@/components/admin-ui', () => ({ NoDataState: () => null }));
