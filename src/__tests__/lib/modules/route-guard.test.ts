 ...
 *   - `@/lib/modules/registry`  → control `isModuleEnabled` per test (keep the
 *     rest of the module REAL via importActual so `ModuleKey` typing is intact).
 *   - `@/lib/domains/tenant`    → control `getSchoolById` (school lookup) per test.
 *   - `@/lib/logger`            → spy so we can assert the warn payload carries
...
vi.mock('@/lib/logger', () => ({ logger: log.logger }));
