 ...
 *   - @/lib/rbac authorizeRequest → always authorized (the institution_admin RBAC
 ...
 *     decision under test.
 *   - @/lib/supabase-admin → returns the caller's school_admins row (carrying the
 *     role we are asserting) + an active school.
 *   - @/lib/feature-flags isFeatureEnabled → ff_school_admin_rbac ON (the test sets
