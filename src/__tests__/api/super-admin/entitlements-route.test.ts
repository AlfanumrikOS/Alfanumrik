 ...
 * module boundary.
 *   - @/lib/admin-auth          → authorizeAdmin (drive authorized/denied) +
 *                                 logAdminAudit (capture audit payloads).
 *   - @/lib/entitlements/resolver → getResolvedEntitlements stubbed so the
 *                                 panel-row builder is deterministic and the
 *                                 test focuses on the ROUTE (auth/validation/
 *                                 write/audit), not resolution (covered by
 *                                 resolver.test.ts).
 *   - @/lib/supabase-admin      → per-table chained builder recording every
 ...
 *   The catalog (@/lib/entitlements/catalog) stays REAL — the route's key/value
...
// ─── @/lib/entitlements/resolver — deterministic resolved set ────────────────
import { ENTITLEMENT_CATALOG } from '@/lib/entitlements/catalog';
...
// ─── @/lib/supabase-admin (per-table chained builder, records writes) ────────
