...
vi.mock('@/lib/learn/daily-rhythm-orchestrator', () => ({ composeDailyRhythm: vi.fn() }));
vi.mock('@/lib/learn/due-reviews-adapter', () => ({ dueReviewsToCards: vi.fn() }));
vi.mock('@/lib/learn/remediation-queue-adapter', () => ({
  ADAPTIVE_REMEDIATION_RULES: { max_remediation_cards_per_day: 3, max_daily_queue_total: 10 },
  compareBySeverity: vi.fn(),
}));
vi.mock('@/lib/goals/goal-profile', () => ({ resolveGoalProfile: vi.fn() }));

import { POST } from '@/app/api/rhythm/today/route';
import { cacheInvalidatePrefixAsync } from '@/lib/cache';
