/**
 * useTeacherCommandCenter — the single client hook both teacher surfaces use to
 * decide whether the Phase 3A "Class Command Center" renders.
 *
 * Two surfaces gate on the SAME flag (`ff_teacher_command_center`):
 *   1. `/teacher` — Command Center home vs the legacy tabbed dashboard.
 *   2. TeacherShell primary nav — slimmed 5-item nav vs the full legacy nav.
 *
 * Flash-avoidance + byte-identical-OFF discipline:
 ...
 *   - A small localStorage cache (1-hour TTL) lets a teacher who HAS the flag
 *     on (post-rollout) skip the legacy→CC flash on repeat visits: the cached
 *     `true` is read synchronously on mount.
 *   - The async `getFeatureFlags` fetch then confirms/corrects and re-caches.
 *
 * Mirrors `useAtlasFlag`'s structure intentionally so the two flag hooks read
 * the same way; kept separate because the default polarity is opposite (Atlas
 * defaults ON in prod, Command Center defaults OFF until rollout).
 */

import { useEffect, useState } from 'react';
import { getFeatureFlags } from './supabase';
import { TEACHER_COMMAND_CENTER_FLAGS } from './feature-flags';

// gitleaks:allow — localStorage key, not a secret.
const CACHE_KEY = 'alfanumrik_teacher_cc_flag_v1'; // gitleaks:allow
...
/**
 * React hook: returns `true` when the Teacher Command Center should render.
 ...
 */
export function useTeacherCommandCenter(): boolean {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const resolved = Boolean(flags[TEACHER_COMMAND_CENTER_FLAGS.V1]);
        writeCache(resolved);
