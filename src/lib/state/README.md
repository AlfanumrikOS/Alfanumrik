...
1. New migration `20260517100000_learner_state_projections.sql` — adds the `learner_mastery` projection table (the clean per-chapter rollup the StudentState model expects).
...
6. [`runtime/event-listener.ts`](./runtime/event-listener.ts) — polling daemon: reads `state_events` since cursor, fans out via dispatcher, advances cursor only on all-subscribers-ok. Standalone runner: `npx tsx scripts/run-event-listener.ts [--dry-run]`.
...
- **Does not depend on Supabase Realtime.** Phase 2 uses polling on the `state_events.occurred_at` cursor. Realtime / pg_notify is a future swap behind the same Dispatcher interface.
