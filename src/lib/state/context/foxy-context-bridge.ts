 ...
 *      build, a missing state_events row, an unparseable event — all
 ...
 *
 * The bridge composes a JourneyEvent[] from `state_events` rows. The
...
/** Max events fetched from state_events (the projector dedupes/drops further). */
...
/**
 * Fetch the learner's recent state_events. Returns Zod-validated
