...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    setGreeting(isHi ? (h < 12 ? 'शुभ प्रभात' : h < 17 ? 'नमस्ते' : 'शुभ संध्या') : (h < 12 ? 'Good morning' : h < 17 ? 'Hello' : 'Good evening'));
  }, [isHi]);

  const loadData = useCallback(async () => {
    if (!student) return;
    const [profs, subs, feats] = await Promise.all([getStudentProfiles(student.id), getSubjects(), getFeatureFlags()]);
    setProfiles(profs); setSubjects(subs); setFlags(feats);
    setNextTopics((await getNextTopics(student.id, student.preferred_subject, student.grade)).slice(0, 3));
    const { count } = await supabase.from('concept_mastery').select('*', { count: 'exact', head: true }).eq('student_id', student.id).lte('next_review_at', new Date().toISOString());
    setDueCount(count ?? 0);
  }, [student]);

  useEffect(() => { if (student) { loadData(); refreshSnapshot(); } }, [student?.id]);
    ...
    <div className="mesh-bg min-h-dvh pb-nav">
      ...
      <header className="sticky top-0 z-40 border-b" style={{ background: 'rgba(251,248,244,0.88)', backdropFilter: 'blur(20px)', borderColor: 'var(--border)' }}>
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setLanguage(language === 'hi' ? 'en' : 'hi')} className="text-xs px-3 py-1.5 rounded-xl border transition-colors" style={{ borderColor: 'var(--border-mid)', color: 'var(--text-3)' }}>
              {language === 'hi' ? '🌐 EN' : '🇮🇳 हिं'}
            </button>
            <button onClick={() => router.push('/profile')}><Avatar name={student.name} /></button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-lg mx-auto px-4 py-4 space-y-4">
        ...
        <Card accent={meta?.color}>
          <div className="flex items-center justify-between mb-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-sm text-[var(--text-2)]">{meta?.name ?? student.preferred_subject} · Grade {student.grade}</span>
              </div>
          ...
          <ProgressBar value={(currentXp % 500) / 500 * 100} label={`${isHi ? 'स्तर' : 'Level'} ${currentLevel}`} showPercent />
          <div className="grid grid-cols-3 gap-2 mt-4">
            ...
            <StatCard value={dueCount} label={isHi ? 'रिव्यू' : 'Due Reviews'} color={dueCount > 0 ? 'var(--orange)' : 'var(--text-3)'} />
          </div>
        </Card>
          ...
          <div>
            ...
            {nextTopics.slice(0, 1).map((topic) => (
              <Card key={topic.id} hoverable onClick={() => router.push('/foxy')} className="flex items-center gap-4 !p-4">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0" style={{ background: `${meta?.color ?? 'var(--orange)'}15` }}>
                  {meta?.icon ?? '📚'}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-sm truncate">{topic.title}</div>
                  <div className="text-xs text-[var(--text-3)] mt-0.5">{isHi ? 'Foxy के साथ सीखो' : 'Learn with Foxy'} · Difficulty {topic.difficulty_level}/5</div>
                </div>
        ...
        {dueCount > 0 && flags.spaced_repetition && (
          <button onClick={() => router.push('/review')} className="w-full rounded-2xl p-4 flex items-center gap-3 transition-all" style={{ background: 'rgba(245,166,35,0.08)', border: '1px solid rgba(245,166,35,0.2)' }}>
            <span className="text-2xl">🔄</span>
            <div className="text-left">
              <div className="font-semibold text-sm" style={{ color: 'var(--gold)' }}>{dueCount} {isHi ? 'रिव्यू बाकी है!' : 'topics due for review!'}</div>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {QUICK_ACTIONS.map((a) => (
              <ActionTile key={a.href} icon={a.icon} label={isHi ? a.labelHi : a.label} color={a.color} onClick={() => router.push(a.href)} />
            ))}
          </div>
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {subjects.slice(0, 8).map((s) => (
                <SubjectChip key={s.code} icon={s.icon} name={s.name} color={s.color} active={student.preferred_subject === s.code} size="sm"
                  onClick={() => { supabase.from('students').update({ preferred_subject: s.code }).eq('id', student.id); router.push(`/learn/${s.code}`); }}
                />
              ))}
            </div>
          </div>
