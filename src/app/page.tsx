...
import { Button, Input, Select, Card, SubjectChip, LoadingFoxy } from '@/components/ui';
...
export default function Home() {
  ...
  const verifyOtp = async () => {
    if (!otp.trim()) return;
    setLoading(true); setError('');
    const { error: e } = await supabase.auth.verifyOtp({ email: email.trim(), token: otp.trim(), type: 'email' });
  ...
  const saveProfile = async () => {
    ...
    if (existing) {
      await supabase.from('students').update({ name, grade, board, preferred_language: lang, onboarding_completed: false }).eq('id', existing.id);
    } else {
      await supabase.from('students').insert({ auth_user_id: user.id, name, grade, board, preferred_language: lang, email: user.email });
    }
    setStep('subject'); setSaving(false);
  };
  ...
  const saveSubject = async () => {
    ...
    await refreshStudent();
    router.replace('/dashboard'); setSaving(false);
  };
    ...
    return (
      <div className="mesh-bg min-h-dvh flex flex-col">
        <div className="flex-1 flex flex-col items-center justify-center px-6 pt-16 pb-8 text-center">
          <div className="text-7xl mb-6 animate-float" style={{ filter: 'drop-shadow(0 4px 20px rgba(232,88,28,0.3))' }}>🦊</div>
          <div
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-6 text-xs font-semibold tracking-wider uppercase"
            style={{ background: 'rgba(232,88,28,0.08)', border: '1px solid rgba(232,88,28,0.2)', color: 'var(--orange)' }}
          >
          ...
          <div className="flex flex-wrap gap-2 justify-center mb-10 max-w-sm">
            {SUBJECT_META.map((s) => (
              <SubjectChip key={s.code} icon={s.icon} name={s.name} color={s.color} />
            ))}
          </div>
          <Button size="lg" fullWidth className="max-w-xs" onClick={() => setStep('auth')}>
            Start Learning Free →
          </Button>
          <p className="text-xs text-[var(--text-3)] mt-3">No credit card. No app download.</p>
        </div>
        <div className="border-t px-6 py-5" style={{ background: 'rgba(255,255,255,0.6)', backdropFilter: 'blur(16px)', borderColor: 'var(--border)' }}>
          <div className="grid grid-cols-3 gap-4 max-w-sm mx-auto text-center">
            {([['2,157+', 'Questions'], ['3,574+', 'RAG Chunks'], ['16', 'Subjects']] as const).map(([val, label]) => (
              <div key={label}>
    ...
    return (
      <div className="mesh-bg min-h-dvh flex items-center justify-center p-6">
        <Card className="w-full max-w-sm animate-slide-up !p-8">
          <button onClick={() => setStep('welcome')} className="text-[var(--text-3)] text-sm mb-6 flex items-center gap-1 hover:text-[var(--text-2)]">← Back</button>
          ...
          {otpSent ? (
            <>
              ...
              <Input className="mb-3 text-center text-2xl tracking-[0.3em]" type="text" placeholder="000000" maxLength={6} value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ''))} onKeyDown={(e) => e.key === 'Enter' && verifyOtp()} />
              {error && <p className="text-red-500 text-sm mb-3">{error}</p>}
              <Button fullWidth className="mb-3" onClick={verifyOtp} disabled={loading || otp.length < 6}>{loading ? 'Verifying…' : 'Verify OTP →'}</Button>
              <Button variant="ghost" fullWidth onClick={() => { setOtpSent(false); setOtp(''); setError(''); }}>Change email</Button>
            </>
          ) : (
            <>
              <Input className="mb-3" type="email" placeholder="your@email.com" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendOtp()} />
              {error && <p className="text-red-500 text-sm mb-3">{error}</p>}
              <Button fullWidth onClick={sendOtp} disabled={loading || !email.trim()}>{loading ? 'Sending…' : 'Send OTP →'}</Button>
            </>
          )}
        </Card>
      </div>
      ...
      <div className="mesh-bg min-h-dvh flex items-center justify-center p-6">
        <Card className="w-full max-w-sm animate-slide-up !p-8">
          ...
          <div className="space-y-3">
            ...
            <Button fullWidth className="mt-2" onClick={saveProfile} disabled={saving || !name.trim()}>{saving ? 'Saving…' : 'Continue →'}</Button>
          </div>
        </Card>
      </div>
    );
  ...
  if (step === 'subject') {
    return (
      <div className="mesh-bg min-h-dvh flex items-center justify-center p-6">
        <Card className="w-full max-w-sm animate-slide-up !p-8">
          <div className="text-4xl mb-4 text-center">📚</div>
          <h2 className="text-2xl font-bold text-center mb-1" style={{ fontFamily: 'var(--font-display)' }}>Pick your main subject</h2>
          ...
          <div className="grid grid-cols-2 gap-2 mb-6">
            {SUBJECT_META.map((s) => (
              ...
              >
                ...
                <div className="text-sm font-semibold" style={{ color: subject === s.code ? s.color : 'var(--text-1)' }}>{s.name}</div>
              </button>
            ))}
          </div>
