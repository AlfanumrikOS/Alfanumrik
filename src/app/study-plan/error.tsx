...
export default function StudyPlanError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    captureException(error, { tags: { boundary: 'study-plan-error', digest: error.digest } });
  }, [error]);
  ...
  return (
    <div className="mesh-bg min-h-dvh pb-nav flex items-center justify-center p-6">
      <div className="text-center max-w-sm">
        <span className="text-5xl block mb-4" role="img" aria-label="Study plan">📚</span>
        <h2 className="text-lg font-bold mb-2" style={{ color: 'var(--text-1)' }}>
          {isHi ? 'स्टडी प्लान लोड नहीं हो सका' : "Study plan couldn't load"}
        </h2>
