...
export default function TodayFocusCard({ item, subjects, isHi }: TodayFocusCardProps) {
  ...
  // Phase 3A Wave A — teacher-assigned items get a visible "from your teacher"
  // tag at the top of the card (the only additive marker; copy/CTA unchanged).
  const teacherAssigned = isTeacherAssigned(item);

  return (
    <Card accent={teacherAssigned ? 'var(--purple)' : 'var(--orange)'} className="!p-5">
      <div className="flex flex-col gap-4">
        {teacherAssigned && (
          <span
            data-testid="today-from-teacher-tag"
            className="inline-flex items-center gap-1.5 self-start rounded-full px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider"
            style={{
