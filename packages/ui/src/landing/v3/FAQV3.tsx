...
/**
 * V3 FAQ — Tailark faqs-component (variant Three) rendering the FULL set of
 * 10 production FAQs lifted from FAQV2.tsx (the e2e pins FAQPage
 * mainEntity.length === 10).
 *
 * - Native <details>/<summary>: zero JS state, keyboard accessible, no
 *   hydration cost (same architecture note as FAQV2).
 * - JSON-LD is English-only, markdown bold stripped (Google policy).
 * - REG-65: the literal "₹699" stays VERBATIM in the plans answer.
 * - COPY FIX (CEO-confirmed production bug): the Unlimited price previously
 *   read ₹1,499/month; corrected to ₹1,099 to match
 *   packages/lib/src/plans.ts PRICING.unlimited.monthly.
 */

type FaqCategory = 'all' | 'parents' | 'pricing' | 'technical';

const CATEGORIES: { key: FaqCategory; en: string; hi: string }[] = [
  { key: 'all', en: 'All', hi: 'सभी' },
  { key: 'parents', en: 'For Parents', hi: 'अभिभावकों के लिए' },
  { key: 'pricing', en: 'Pricing', hi: 'मूल्य' },
  { key: 'technical', en: 'Technical', hi: 'तकनीकी' },
];

const FAQS: { qEn: string; qHi: string; aEn: string; aHi: string; category: FaqCategory }[] = [
  {
  ...
  },
  {
    ...
    // REG-65 pins "₹699" verbatim. Unlimited corrected ₹1,499 → ₹1,099
    // (matches PRICING.unlimited.monthly; CEO-confirmed copy fix).
    // SUBJECT-COUNT COPY FIX (2026-08-11): this answer used to gate subjects by
    // tier — "across 2 subjects" (Explorer), "4 subjects" (Starter), "all
    // subjects" (Pro). Migration 20260814000018 made all three false:
    // max_subjects is NULL on every plan and plan_subject_access grants all
    // five codes to every plan. Because this array is ALSO the source of the
    // FAQPage JSON-LD below, the false tiering was being cached off-site in
    // search results. Not one rupee figure changed in this edit.
...
export default function FAQV3() {
  ...
  return (
    <section className={s.section} id="faq" aria-labelledby="faq-v3-title">
      <div className={s.wrap}>
        <div className={s.sectionHead}>
          <span className={s.eyebrow}>{t('FAQ', 'सामान्य प्रश्न')}</span>
          <h2 id="faq-v3-title">
            {t('Your questions, answered', 'आपके प्रश्न, उत्तर सहित')}
          </h2>
        </div>
        <div className={s.faqTabs} role="tablist" aria-label={t('FAQ categories', 'FAQ श्रेणियाँ')}>
            {CATEGORIES.map((cat) => (
              <button
                key={cat.key}
                type="button"
                ...
                className={activeCategory === cat.key ? `${s.faqTab} ${s.faqTabActive}` : s.faqTab}
        ...
        <div className={s.faqWrap}>
          {FAQS.filter(faq => activeCategory === 'all' || faq.category === activeCategory).map((faq, i) => (
            <Fragment key={i}>
              <details
                className={s.faqItem}
                onToggle={(e) => {
                  // Fire only on open, not on close (same rule as V2).
              ...
              >
                ...
                <div className={s.faqBody}>
                  <p lang={isHi ? 'hi' : undefined}>
                    {(isHi ? faq.aHi : faq.aEn).replace(/\*\*/g, '')}
                  </p>
                </div>
              </details>
              {/* Hairline after every item except the last; the module rule
                  .faqItem[open] + .faqDivider fades it while the item is open. */}
