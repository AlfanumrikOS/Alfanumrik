...
import { useResponsiveCanvas } from '@alfanumrik/lib/hooks/useResponsiveCanvas';
...
export default function AtomBuilder() {
  ...
  useEffect(() => {
    ...
    // Nucleus
    const nr = Math.max(14, (protons + neutrons) * 0.4);
    for (let i = 0; i < protons + neutrons; i++) {
      ...
      ctx.fillStyle = i < protons ? '#ef4444' : '#9ca3af';
      ctx.fill();
    }
    ctx.beginPath();
  ...
  }, [protons, neutrons, electrons, size, canvasRef]);
    ...
    <div style={{ background: 'var(--surface-1)', borderRadius: 12, padding: 16, maxWidth: 600, margin: '0 auto', fontFamily: 'inherit' }}>
      ...
      <div style={{ marginTop: 8, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 120, padding: '8px 12px', background: 'var(--surface-2)', borderRadius: 8, fontSize: 12 }}>
          ...
          <div style={{ fontWeight: 700, color: 'var(--text-1)', fontSize: 14 }}>{protons > 0 && protons <= 30 ? name : 'Unknown'}</div>
        ...
        <div style={{ flex: 1, minWidth: 120, padding: '8px 12px', background: 'var(--surface-2)', borderRadius: 8, fontSize: 12 }}>
          ...
          <div style={{ fontWeight: 700, color: '#60a5fa' }}>{fillShells(electrons).join(' | ') || 'None'}</div>
