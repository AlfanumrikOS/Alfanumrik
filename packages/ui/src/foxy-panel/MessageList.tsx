...
import { DIRECTIVE_ECHO_LABELS } from './foxy-constants';
import type { QuizMeBinding } from '@alfanumrik/ui/foxy/FoxyStructuredRenderer';
import type { SubmitQuizAnswerInput, SubmitQuizAnswerResult } from './useFoxyChat';
...
/**
 * Any `ui_action` payload extracted from legacy ```json ...``` blobs in a
 * tutor bubble's `content` — the shape is defined by the /foxy page's
 * DynamicScaffold module and the panel is intentionally scaffold-agnostic.
 */
...
export interface MessageListProps {
  ...
  /**
   * Optional render-prop for the legacy `ui_action` scaffold panel. The full
   * /foxy page passes a renderer that mounts DynamicScaffold (dynamic import
   * kept OFF the shared bundle). Embed contexts (dashboard, learn, quiz
   * results) omit this; a `ui_action` payload then renders nothing (byte-safe
   * — the scaffold was only shown on the full page anyway).
   */
  renderScaffold?: (action: UiActionPayload) => ReactNode;
}
          ...
          <div key={msg.id}>
            <ChatBubble
              ...
              content={msg.role === 'tutor' ? (
                <div>
                  {tutorContent}
                  {uiActionPayload && renderScaffold ? renderScaffold(uiActionPayload) : null}
                </div>
              ) : (
                <div>
                  {msg.imageUrl && (
