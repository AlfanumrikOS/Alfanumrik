 ...
 *
 * Usage:
 *   import { createDelegationToken, validateDelegationToken, revokeDelegationToken } from '@alfanumrik/lib/rbac-delegation';
 *
...
import { getUserPermissions, invalidateForSecurityEvent } from '@alfanumrik/lib/rbac';
...
export async function createDelegationToken(
  ...
  try {
    ...
    try {
      const { writeAuditEvent } = await import('@alfanumrik/lib/audit-pipeline');
