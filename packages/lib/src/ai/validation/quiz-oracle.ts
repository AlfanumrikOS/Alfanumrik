...
export function runDeterministicChecks(
  ...
  // String-only enum (A3 — REG-54 follow-up). Integer 1..5 was the legacy
  // shape but `question_bank` schema and `exam-engine.ts` only speak in
  // string enums, and the integer path was dead in callers. Keeping it
  // around invited drift, so it's gone.
  ...
  // ── 6b. grade (when provided, A4) ─────────────────────────────────────────
  // P5 invariant: grades are strings "6".."12". The generator pins a grade
  // on input but the candidate could echo something else — verify when the
  ...
  // ── 6c. subject (when provided, A4) ───────────────────────────────────────
  // CBSE subject allowlist. The oracle only verifies the subject is a known
  // code; grade-by-grade subject gating remains the caller's responsibility.
