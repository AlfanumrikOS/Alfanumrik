/**
 * ALFANUMRIK — XP Economy Rules (Live Source)
 *
 * This is the canonical source for XP earning values, caps, level math, level
 * names, and the legacy XP redemption catalog. The historical `xp-rules.ts`
 * module is now a thin re-export shim that points here so that the existing
 * import paths continue to work while the codebase migrates.
 *
 ...
 *
 * All XP constants live here. There must be no hardcoded XP values anywhere
 * else in the codebase. Daily quiz cap: 200 XP. Level: 500 XP.
 *
 * The constants below MUST stay in lock-step with:
 *   - `submitQuizResults()` in `src/lib/supabase.ts`
 *   - `calculateQuizXP()` in `src/lib/scoring.ts`
 *   - `atomic_quiz_profile_update()` RPC in `supabase/migrations/`
 */
...
/** @see {@link import('@alfanumrik/lib/score-config').getLevelFromScore} */
export { getLevelFromScore } from './score-config';

/** @see {@link import('@alfanumrik/lib/coin-rules').COIN_REWARDS} */
/** @see {@link import('@alfanumrik/lib/coin-rules').COIN_SHOP} */
