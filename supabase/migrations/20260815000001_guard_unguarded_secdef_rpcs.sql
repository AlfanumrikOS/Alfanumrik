-- Migration: 20260815000001_guard_unguarded_secdef_rpcs.sql
-- Purpose: Close an IDOR gap on SECURITY DEFINER RPCs that accept a
-- caller-supplied p_student_id / p_guardian_id but never verified the
-- caller's JWT actually owns that row. Any authenticated user could call
-- these directly (e.g. via supabase-js RPC) with an arbitrary UUID and read
-- another student's notifications/snapshot/reviews, join another student
-- into a class/competition, or read another guardian's dashboard.
--
-- Fix pattern: reproduce each function body verbatim from the baseline
-- (00000000000000_baseline_from_prod.sql; link_guardian_to_student_via_code
-- taken from its latest override, 20260720170000_parent_dashboard_rca_fixes.sql)
-- and insert a single ownership guard as the first statement inside BEGIN.
-- The guard is a no-op (`auth.uid() IS NOT NULL AND ...`) when auth.uid() is
-- NULL, i.e. when the caller is a service-role/server-side invocation with
-- no JWT (Postgres session has no request.jwt.claim.sub) -- this preserves
-- all existing server-side call sites (API routes using supabase-admin,
-- Edge Functions, cron) while blocking any authenticated end-user session
-- from impersonating a different student/guardian id.
--
-- generate_exam_paper already had an ownership guard, but it was missing
-- the `auth.uid() IS NOT NULL AND` prefix, so it unconditionally rejected
-- ANY caller with no JWT -- including the /api/quiz route's service-role
-- caller. This migration normalizes it to the same no-op-when-NULL form
-- used everywhere else (live-bug fix, not a new restriction). It also adds
-- `USING ERRCODE = '42501'` to that function's RAISE EXCEPTION (harmless --
-- same exception, now matching the SQLSTATE convention used by the other
-- 8 guards in this migration).
--
-- generate_student_notifications has no per-caller p_student_id ownership
-- concept to check (it fans out notifications for one student_id it is
-- told to process, with no return of another student's data) but it must
-- never be directly callable by an authenticated end user with an
-- arbitrary p_student_id (it would let any signed-in user trigger
-- notification generation, and read the row-count side channel, for any
-- other student). It is invoked only by daily-cron; grants are restricted
-- to service_role only, no body change needed.

BEGIN;

-- ---------------------------------------------------------------------------
-- 1. get_student_notifications(uuid, integer) -- STUDENT-ID guard
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. get_student_snapshot(uuid) -- STUDENT-ID guard
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. get_review_cards(uuid, integer) -- STUDENT-ID guard
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. student_join_class(uuid, text) -- STUDENT-ID guard
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5. join_competition(uuid, uuid) -- STUDENT-ID guard
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 6. get_guardian_dashboard(uuid) -- GUARDIAN-ID guard
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION "public"."get_guardian_dashboard"("p_guardian_id" "uuid") RETURNS "jsonb"
  ...
  IF auth.uid() IS NOT NULL AND NOT EXISTS (SELECT 1 FROM guardians WHERE id = p_guardian_id AND auth_user_id = auth.uid()) THEN
    RAISE EXCEPTION 'Access denied' USING ERRCODE = '42501';
  END IF;
...
-- ---------------------------------------------------------------------------
-- 7. link_guardian_to_student_via_code(uuid, text) -- GUARDIAN-ID guard
--    Body reproduced from the latest override
--    (20260720170000_parent_dashboard_rca_fixes.sql), which widened the
--    code match to invite_code OR link_code. Only the guard is new here.
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION "public"."link_guardian_to_student_via_code"("uuid", "text")
  IS 'Parent-portal OTP-redeem link RPC. Matches students.invite_code OR students.link_code (fixed 2026-07-20 - previously matched invite_code only, causing a false Invalid invite code rejection for codes issued via link_code after the OTP challenge had already been verified). Sets status=active, matching public.is_guardian_of() which treats active/approved as linked. Guarded 2026-08-15 against p_guardian_id impersonation by an authenticated caller who does not own that guardian row.';
...
-- ---------------------------------------------------------------------------
-- 8. generate_exam_paper(uuid, text, text, integer[], uuid) -- normalize the
--    existing guard: add `auth.uid() IS NOT NULL AND` so a service-role
--    caller with no JWT (e.g. /api/quiz) is not unconditionally rejected.
--    This is a live-bug fix, not a new restriction: end-user-owned calls
--    are guarded exactly as before.
-- ---------------------------------------------------------------------------
...
BEGIN
  IF auth.uid() IS NOT NULL AND NOT EXISTS (SELECT 1 FROM students WHERE id = p_student_id AND auth_user_id = auth.uid()) THEN RAISE EXCEPTION 'Access denied' USING ERRCODE = '42501'; END IF;
...
-- ---------------------------------------------------------------------------
-- 9. generate_student_notifications(uuid) -- no body change (no
--    ownership predicate applies: it fans out notifications for the
--    student_id it is given, it does not read/return another student's
--    data via a mismatched id the way the others above did). Restrict
--    grants to service_role only -- this is invoked by daily-cron and must
--    never be directly callable by an authenticated end user.
-- ---------------------------------------------------------------------------
