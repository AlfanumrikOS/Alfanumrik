-- Fix: lower the Bloom-diversity gate from >= 3 to >= 1 so that chapters
-- with a single assessed bloom category (e.g. all 'remember' questions) can
-- still be marked complete once accuracy >= 60%.
--
-- Root cause: migration 20260609000000_lesson_flow_and_parameters.sql raised
-- the gate to >= 3 to enforce multi-bloom assessment, but question_bank currently
-- has ≤5 'remember'-only MCQ rows per chapter → assessed_count always = 1 →
-- is_completed never flips → the resolver loops on the same chapter forever.
--
-- TODO(assessment): Raise back to assessed_count >= 3 via a follow-up migration
-- (supabase/migrations/20260700000001_raise_bloom_diversity_gate.sql) after
-- the bulk MCQ seeding script (scripts/bulk-mcq-driver.ts) has added
-- 'understand' and 'apply' questions to every chapter.
...
BEGIN
  ...
  -- Mark chapter complete: overall accuracy >= 60% AND at least 1 bloom parameter assessed.
  -- CHANGED from >= 3: question_bank currently has only 'remember'-level MCQs per chapter,
  -- so assessed_count = 1 always. Raise back to >= 3 after bulk MCQ seeding adds
  -- 'understand' and 'apply' questions (see TODO above).
