-- Fix: add a minimum pool size guard to the 80% pool-reset in both question
-- selection RPCs. Previously, a chapter with ≤5 questions would reset history
-- on every single call (5/5 = 100% >= 80%) → infinite cycle of the same questions.
--
-- Now: reset only fires when total_pool >= 10 AND seen/total >= 0.80.
-- Chapters with < 10 questions serve all available questions (ranked by
-- least-recently-seen) without resetting history.
