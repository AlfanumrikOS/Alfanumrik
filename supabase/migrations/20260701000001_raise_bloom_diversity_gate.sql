-- Migration: 20260701000001_raise_bloom_diversity_gate.sql
--
-- Raises the Bloom-diversity gate in update_chapter_progress from >= 1 back
-- to >= 3. This is the counterpart to migration 20260625000100 which
-- temporarily lowered the gate to unblock chapter completion while the
-- question_bank had only 'remember'-level MCQs.
--
-- APPLY ONLY AFTER scripts/bulk-mcq-driver.ts has completed seeding
-- 'understand' and 'apply' questions across all chapters. Verify with:
--
--   SELECT subject, grade, chapter_number,
--          COUNT(*) FILTER (WHERE bloom_level='remember') AS r,
--          COUNT(*) FILTER (WHERE bloom_level='understand') AS u,
--          COUNT(*) FILTER (WHERE bloom_level='apply') AS a
--   FROM question_bank
--   WHERE is_active = true AND question_type_v2 = 'mcq'
--   GROUP BY subject, grade, chapter_number
--   HAVING COUNT(*) FILTER (WHERE bloom_level='understand') < 4
--      OR  COUNT(*) FILTER (WHERE bloom_level='apply') < 4
--   LIMIT 10;
--
-- That query should return 0 rows before applying this migration.
-- If it returns rows, run the driver for those chapters first.
--
-- Tracked by: TODO comment in 20260625000100_fix_chapter_completion_threshold.sql
...
BEGIN
  ...
  -- Mark chapter complete: overall accuracy >= 60% AND at least 3 bloom parameters assessed.
  -- RESTORED from >= 1: bulk MCQ seeding (scripts/bulk-mcq-driver.ts) has added
  -- 'understand' and 'apply' questions to every chapter, so assessed_count can
  -- now realistically reach 3. Migration 20260625000100 temporarily set this to
  -- >= 1 while seeding was pending.
