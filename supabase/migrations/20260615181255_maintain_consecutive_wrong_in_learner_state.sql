-- Adds consecutive_wrong maintenance to update_learner_state_post_quiz.
-- Extends the existing concept_mastery upsert only (INSERT VALUES + ON CONFLICT DO UPDATE SET).
-- BKT/SM-2 outputs are provably unchanged (consecutive_wrong feeds no formula).
-- Column added in 0a3b46ef (20260615180149); population logic ships here.

-- Idempotent: DROP FUNCTION IF EXISTS (exact arg-type signature) + CREATE OR REPLACE.
-- Body reproduced byte-for-byte from 20260615142552_restore_missing_quiz_functions.sql
-- (the deployed version, restored in c01f77ee). Only the 3 consecutive_wrong spots
-- in the concept_mastery upsert differ; all BKT/SM-2 logic, the mastery_level::TEXT
-- write, the RETURN jsonb, the 10-param signature, and SECURITY DEFINER + search_path
-- are unchanged.
...
COMMENT ON FUNCTION update_learner_state_post_quiz IS
  'Atomically updates BKT mastery, error counts, retention, bloom mastery, streak, consecutive_wrong, and CME action after a quiz attempt.';
