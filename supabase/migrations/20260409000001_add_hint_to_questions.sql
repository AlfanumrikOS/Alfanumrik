...
-- Purpose: Add hint and hint_hi columns to question_bank table (idempotent — legacy
...
ALTER TABLE question_bank ADD COLUMN IF NOT EXISTS hint_hi TEXT;
...
CREATE INDEX IF NOT EXISTS idx_question_bank_difficulty_no_hint
  ON question_bank (difficulty DESC, id)
  WHERE hint IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_question_bank_has_hint
  ON question_bank (id)
...
--     AND indexname IN (
--       'idx_question_bank_difficulty_no_hint',
--       'idx_question_bank_has_hint'
--     );
-- Expected: 2 rows
-- ─────────────────────────────────────────────────────────────────────────────
