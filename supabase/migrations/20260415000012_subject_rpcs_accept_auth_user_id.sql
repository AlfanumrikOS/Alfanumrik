-- supabase/migrations/20260415000012_subject_rpcs_accept_auth_user_id.sql
--
-- CRITICAL PRODUCTION BUG FIX.
--
-- Symptom: Every student dashboard showed 0 subjects after the Phase B
-- shipping. `GET /api/student/subjects` returned empty, ReselectBanner did
-- not show (because allowedSubjects had length 0 instead of >0), and the
-- "My Subjects" chip row was blank.
--
-- Root cause: /api/student/subjects passes `auth.uid()` (auth.users.id) to
-- get_available_subjects(p_student_id). The RPC looked up students WHERE id
-- = p_student_id. But students.id != students.auth_user_id — the join
-- column between auth and the students table is auth_user_id, not id.
-- Result: RPC always returned 0 rows on the happy path.
--
-- Fix: both get_available_subjects and set_student_subjects now resolve
-- p_student_id against EITHER students.id (legacy admin callers) OR
-- students.auth_user_id (API + client callers passing auth.uid()).
--
-- Also updates set_student_subjects authz to compare auth.uid() against
-- students.auth_user_id (the correct identity) instead of students.id.
--
-- Applied to prod via mcp apply_migration 2026-04-15; file committed for
-- CI parity.
