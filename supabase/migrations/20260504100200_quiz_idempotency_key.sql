...
-- Purpose:    Marking-Authenticity Phase 2.8 — idempotent quiz submission.
--             Add idempotency_key to quiz_sessions and extend
--             submit_quiz_results_v2 to short-circuit on replay so a network
--             retry from the client never produces a duplicate quiz_sessions
--             row, duplicate XP grant, or duplicate atomic_quiz_profile_update.
--
-- Threat model:
--   Today the v2 RPC is called from the (Wave 2) /api/quiz/submit Next.js
--   route. If the client retries on a transient 5xx but the original call
--   actually committed, the student gets two quiz_sessions rows + double XP.
--   The fix: per-(student_id, idempotency_key) uniqueness on quiz_sessions,
--   plus a "look first, return cached result" branch in the RPC that returns
--   the original score/xp shape with `idempotent_replay: true`.
--
-- Schema change (safe / reversible):
--   ALTER TABLE quiz_sessions ADD COLUMN idempotency_key UUID NULL;
--     - NULLABLE for backfill safety: every existing row stays NULL.
--     - No default value: pre-existing inserts continue to work; only
--       the v2 RPC populates this column going forward.
--   UNIQUE INDEX quiz_sessions_idempotency_key_uniq
--     ON quiz_sessions(student_id, idempotency_key)
--     WHERE idempotency_key IS NOT NULL;
--     - Partial index lets every legacy NULL row coexist while still
--       enforcing uniqueness for any row that does set the key.
--
-- RPC contract change:
--   New optional last parameter p_idempotency_key UUID DEFAULT NULL.
--   Existing callers (no key supplied) get the default and run the legacy
--   path — no behavior change for them. Wave 2 callers passing a UUID get
--   the new short-circuit:
--     1. Look up (student_id, idempotency_key) in quiz_sessions.
--     2. If found, RETURN the cached shape with `idempotent_replay: true`.
--     3. If not found, run the normal scoring path AND persist the key on
--        the new quiz_sessions row. The unique index closes the race
--        between two concurrent retries.
--
-- Return shape addition:
--   `idempotent_replay BOOLEAN` — defaults to false on every code path.
--   Wave 2 / mobile clients can treat true as "this was a replay; do not
--   re-animate XP gain, just navigate to results".
--
-- Backwards compatibility:
--   - We DROP the prior submit_quiz_results_v2 signature
--     (UUID, UUID, TEXT, TEXT, TEXT, INTEGER, JSONB, INTEGER) and re-create
--     with the extended one
--     (..., INTEGER, UUID).  PostgreSQL function overload resolution prefers
--     the most specific match, so callers that omit the new arg fall through
--     the DEFAULT NULL — no signature update required at any call site.
--   - submit_quiz_results (v1) is untouched (mobile + legacy paths).
--   - The body is the Phase 1.2 version verbatim (silent-zero RAISE), with
--     two additions: (a) the idempotency lookup at the top, and
--     (b) a single line in the INSERT that persists the key.
--
-- Idempotent (this migration):
--   ADD COLUMN IF NOT EXISTS, CREATE UNIQUE INDEX IF NOT EXISTS,
--   DROP FUNCTION IF EXISTS, CREATE OR REPLACE FUNCTION.
--
-- Reversible:
--   - Drop the unique index (safe).
--   - Drop the column (safe; legacy callers never read it).
--   - Re-apply migration 20260504100100 to restore the prior RPC signature.
...
COMMENT ON COLUMN public.quiz_sessions.idempotency_key IS
  'Marking-Authenticity Phase 2.8: per-student idempotency token supplied by '
  'the v2 quiz submit caller. NULL for legacy rows and for v1 submissions. '
  'When non-NULL, the partial unique index quiz_sessions_idempotency_key_uniq '
  'guarantees a single quiz_sessions row per (student_id, idempotency_key), '
  'so client retries on transient 5xx no longer produce duplicate XP grants.';

CREATE UNIQUE INDEX IF NOT EXISTS quiz_sessions_idempotency_key_uniq
  ON public.quiz_sessions (student_id, idempotency_key)
  WHERE idempotency_key IS NOT NULL;

-- ───────────────────────────────────────────────────────────────────────────
-- 2. RPC: drop the prior overload, re-create with the extra optional param
-- ───────────────────────────────────────────────────────────────────────────

-- DROP IF EXISTS so re-applying this migration is a no-op even after a
-- previous successful run. The CREATE OR REPLACE below cannot extend an
-- existing function's parameter list, so DROP + CREATE is required.
...
-- End of migration: 20260504100200_quiz_idempotency_key.sql
