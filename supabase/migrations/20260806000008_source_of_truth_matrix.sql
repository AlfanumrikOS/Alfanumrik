...
VALUES
  ...
  ('Quiz response submitted and scored',
   ...
   'append_only (corrections via new events, never overwrite)',
   ARRAY['concept_mastery', 'xp_transactions', 'student_learning_profiles', 'state_events'],
   'transactional (session + responses + XP in single RPC)',
  ...
  ('Recommendation generated for learner',
   ...
   'scheduled (runs on learner loop trigger or cron)',
  ...
  ('Foxy AI interaction recorded',
   ...
   'transactional (message + trace in one request)',
  ...
  ('RAG content chunk ingested and searchable',
   ...
   'versioned (content_id + version + chunker_version)',
   ARRAY['rag_content_documents', 'cbse_syllabus', 'rag_retrieval_logs', 'pgvector indexes'],
   'scheduled (batch ingestion via scripts)',
  ...
  ('Safety incident escalated',
   ...
   'append_only (resolution events are separate rows, never overwrite)',
  ...
  ('Subscription payment processed',
   ...
   'append_only (refunds are separate rows)',
   ARRAY['subscription_events', 'payment_webhook_events', 'plan_limit_coverage'],
   'transactional (webhook handler writes payment + subscription atomically)',
  ...
  ('Question bank question verified and approved',
   ...
   'versioned (content_version_id tracks edits)',
