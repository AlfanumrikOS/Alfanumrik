...
//
// CANONICAL Deno twin of packages/lib/src/ai/validation/output-screen.ts
// (FOX-1, P12). Relocated here from grounded-answer/output-screen.ts
// (forensic audit HIGH-2, 2026-07-29) so every Deno Edge Function that
// returns AI-generated text to a student — not just grounded-answer's
// streaming path — can share ONE screening implementation instead of each
// function growing its own (or, as the audit found, some functions growing
// none at all). grounded-answer/output-screen.ts now re-exports from here
// for backward compatibility with its existing imports/tests.
//
// Deterministic, fail-safe content backstop for student-facing AI text. The
// Next-TS and Deno-TS module graphs can't share a file, so this mirrors the
// TS `screenStudentFacingText` blocking set verbatim. Keep the
// HARD_BLOCK_PATTERNS list BYTE-FOR-BYTE in sync with the TS twin.
//
...
//   The prompt-level FOXY_SAFETY_RAILS / solver system-prompt rules + the
//   grade/subject scope + grounding cover the borderline-curriculum terms.
//
...
//
// On the grounded-answer streaming path this lets us yield an `abstain` event
// INSTEAD of `done` when the completed (buffered) answer is unsafe, so the
// unsafe structured/done frame never reaches the client. Other callers (e.g.
// ncert-solver) use it as a hard gate before constructing their success
// response — see each caller for its own fallback shape.
