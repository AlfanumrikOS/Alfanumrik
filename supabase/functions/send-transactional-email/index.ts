...
// ─── Main Handler ─────────────────────────────────────────────────────────────
type TemplateName = 'school-trial-provisioned' | 'school-invite-code-issued'
...
Deno.serve(async (req: Request) => {
  ...
  if (!MAILGUN_API_KEY || !MAILGUN_DOMAIN) {
    console.warn(`[Transactional Email] Mailgun not configured. Skipping send for template=${template} code=${truncateCode(params.invite_code)}`)
  ...
  try {
    ...
    if (result.success) {
      console.log(`[Transactional Email] Sent template=${template} code=${truncateCode(params.invite_code)} to=${redactedEmail} id=${result.id}`)
      return jsonResponse({ sent: true, id: result.id }, 200, origin)
    }
    console.error(`[Transactional Email] Mailgun error template=${template} code=${truncateCode(params.invite_code)}: ${result.error}`)
