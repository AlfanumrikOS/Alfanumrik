/**
 * TOMBSTONE — rag-ingest-status retired 2026-08-31.
 *
 ...
 * `*.ts`, `*.tsx`, `*.dart`, `*.sql` found zero call sites. Companion to
 * `rag-ingest-batch` (also tombstoned in this same sweep).
 *
 ...
 * Permanent deletion (`supabase functions delete rag-ingest-status`) is a
...
serve((req) => {
  ...
  console.warn(`[rag-ingest-status:tombstone] method=${req.method} ua="${userAgent}"`)

  return new Response(
    JSON.stringify({
      code: 'GONE',
      error: `The rag-ingest-status Edge Function was retired on ${RETIRED_ON}. It had zero application callers.`,
    }),
    {
      status: 410,
