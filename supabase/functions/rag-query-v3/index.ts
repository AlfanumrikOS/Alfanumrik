/**
 * TOMBSTONE — rag-query-v3 retired 2026-08-31.
 *
 ...
 * naming by later `rag-answer-v3/v4/v5` (also tombstoned in this same
 * sweep — none of them have live callers either).
 *
 ...
 * Permanent deletion (`supabase functions delete rag-query-v3`) is a
...
serve((req) => {
  ...
  console.warn(`[rag-query-v3:tombstone] method=${req.method} ua="${userAgent}"`)

  return new Response(
    JSON.stringify({
      code: 'GONE',
      error: `The rag-query-v3 Edge Function was retired on ${RETIRED_ON}. It had zero application callers.`,
    }),
    {
      status: 410,
