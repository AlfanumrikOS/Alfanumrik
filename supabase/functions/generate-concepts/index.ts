...
// Phase 2 continued (2026-05-24): Python AI services cutover proxy. When
// ff_python_generate_concepts_v1 is enabled AND the per-request bucket
...
Deno.serve(async (req: Request) => {
  ...
  try {
    // ── Python AI services cutover (Phase 2 continued — 2026-05-24) ─────────
    // When ff_python_generate_concepts_v1 is enabled AND the per-request
    ...
    //
    // Rollback gate is identical to ff_python_generate_answers_v1: if
    // Cloud Run is returning bad results, ops flips metadata.kill_switch=
    // true and the 5-min flag cache TTL drains the proxy without a
    // redeploy. If Cloud Run is hard-down, the proxy throws and we fall
    // through to the TS path — admin batch jobs never see a 502 because
    ...
    const proxyTraceId =
      ...
      (typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function'
        ? crypto.randomUUID()
        : `gen-cpt-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`)

    const proxyDecision = await shouldProxyToPython({
      flag_name: 'ff_python_generate_concepts_v1',
      endpoint_path: '/v1/generate-concepts',
    ...
    if (proxyDecision.should_proxy && proxyDecision.target_url) {
      try {
        ...
        console.warn(
          `[python-ai-proxy] forward failed for generate-concepts: ${err instanceof Error ? err.message : String(err)}; falling back to TS path`,
        )
