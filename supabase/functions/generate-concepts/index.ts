...
const GENERATE_CONCEPTS_ROUTE_PROFILE = createStaticAiRouteProfile({
  route: 'generate-concepts',
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.warn('[generate-concepts] python proxy check failed:', err instanceof Error ? err.message : String(err))
  }
  ...
  const admitResult = await admitAiRoute({ req, sb, profile: GENERATE_CONCEPTS_ROUTE_PROFILE, bodyText })
