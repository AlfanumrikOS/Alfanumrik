/**
 * TOMBSTONE — rag-answer-v5 retired 2026-08-31.
 *
 ...
 * `*.ts`, `*.tsx`, `*.dart`, `*.sql` found zero call sites. Production
 * answer-grounding runs through `grounded-answer` (live, git-tracked,
 * unaffected).
 *
 ...
 * Permanent deletion (`supabase functions delete rag-answer-v5`) is a
...
serve((req) => {
  ...
  console.warn(`[rag-answer-v5:tombstone] method=${req.method} ua="${userAgent}"`)

  return new Response(
    JSON.stringify({
      code: 'GONE',
      error: `The rag-answer-v5 Edge Function was retired on ${RETIRED_ON}. It had zero application callers. Use grounded-answer instead.`,
    }),
    {
      status: 410,
