...
export default function ParentV3PageGate({ legacy, v3 }: { legacy: React.ReactNode; v3: React.ReactNode }) {
  const { enabled, loading, manifest, routeAllowed, legacyAllowed, denied } = useExperienceV3('parent');
  const parentAuth = useParentAuth();
  if (loading || parentAuth.loading) return null;
  if (legacyAllowed) return <>{legacy}</>;
  if (denied || !enabled || parentAuth.mode !== 'guardian' || !manifest || !routeAllowed) return <DataState state="permission" title="This parent destination is unavailable" />;
  return <>{v3}</>;
}
