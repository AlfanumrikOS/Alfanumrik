    ...
    >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline justify-between gap-3">
            <div
              className={[
                'truncate text-[14px]',
                item.is_read ? 'font-medium text-[color-mix(in_srgb,var(--foreground)_80%,transparent)]' : 'font-semibold text-foreground',
              ].join(' ')}
            >
              {item.title}
            </div>
      ...
      {expanded && (
        <div className="mt-2 pl-5">
          <p className="whitespace-pre-line text-[13px] leading-relaxed text-[color-mix(in_srgb,var(--foreground)_90%,transparent)]">
            {longBody}
          </p>
          {!item.is_read && (
            <button
