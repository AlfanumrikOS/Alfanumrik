...
import { CONSUMER_PRICING_PAISA, type ConsumerPlanCode } from '@alfanumrik/lib/pricing';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Anti-abuse: cap order creation per authenticated user — a compromised
    // session should not be able to create unlimited Razorpay orders.
    ...
    // `CONSUMER_PRICING_PAISA` (= plans.ts PRICING × 100) in `@alfanumrik/lib/pricing`.
