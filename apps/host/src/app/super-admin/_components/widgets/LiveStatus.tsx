...
import { StatCard, StalenessTag } from '@alfanumrik/ui/admin-ui';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
          <StatCard label="Students" value={stats.totals.students} accentColor="#2563EB" />
          <StatCard label="Teachers" value={stats.totals.teachers} accentColor="#16A34A" />
      ...
      {stats && (
        <div className="rounded-lg border border-surface-3 bg-surface-1 p-3">
          <div style={{ fontSize: 11, fontWeight: 700, color: '#6B7280', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Activity</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
            <div style={{ borderRight: '1px solid #E5E7EB', paddingRight: 12 }}>
              <div style={{ fontSize: 10, color: '#9CA3AF', fontWeight: 600, marginBottom: 4 }}>LAST 24H</div>
            ...
            {stats.last_7d && (
              <div style={{ paddingLeft: 12 }}>
                <div style={{ fontSize: 10, color: '#9CA3AF', fontWeight: 600, marginBottom: 4 }}>LAST 7D</div>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 10, paddingTop: 10, borderTop: '1px solid #F3F4F6' }}>
            <StatCard label="Quiz Sessions" value={stats.totals.quiz_sessions} accentColor="#D97706" />
      ...
      <div className="rounded-lg border border-surface-3 bg-surface-1 p-3">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#6B7280', textTransform: 'uppercase', letterSpacing: 1 }}>Feature Flags</div>
          <a href="/super-admin/flags" style={{ fontSize: 11, color: '#2563EB', textDecoration: 'none' }}>Manage</a>
        </div>
        ...
        {flags.length === 0 && <div style={{ fontSize: 11, color: '#9CA3AF' }}>No flags configured</div>}
      </div>

      {/* Subscription Breakdown */}
      {analytics && analytics.revenue.length > 0 && (
        <div className="rounded-lg border border-surface-3 bg-surface-1 p-3">
          <div style={{ fontSize: 11, fontWeight: 700, color: '#6B7280', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Subscription Plans</div>
          {analytics.revenue.map(r => {
            ...
            return (
              <div key={r.plan} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                <span style={{ fontSize: 11, color: '#6B7280', width: 100, textTransform: 'capitalize', flexShrink: 0 }}>{r.plan.replace(/_/g, ' ')}</span>
