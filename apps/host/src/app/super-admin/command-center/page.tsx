        ...
        <div>
          ...
          {rec.status === 'proposed' && (
            <div style={{ borderTop: `1px solid ${'#E5E7EB'}`, paddingTop: 14, marginTop: 14, display: 'flex', gap: 10 }}>
              ...
              <button
                className="rounded-md border border-danger bg-[color-mix(in_srgb,var(--danger)_10%,transparent)] px-4 py-2 text-sm font-semibold text-danger hover:bg-[color-mix(in_srgb,var(--danger)_20%,transparent)]"
        ...
        <div>
          ...
          <div style={{ borderTop: `1px solid ${'#E5E7EB'}`, paddingTop: 14, marginTop: 14, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            {exec.status === 'deployed' && (
              <button className="rounded-md border border-danger bg-[color-mix(in_srgb,var(--danger)_10%,transparent)] px-4 py-2 text-sm font-semibold text-danger hover:bg-[color-mix(in_srgb,var(--danger)_20%,transparent)]" disabled={!!pipelineActionLoading[exec.id]} onClick={() => { handleRollback(exec.id); setDrawerOpen(false); }}>Rollback</button>
            )}
          </div>
        </div>
      );
