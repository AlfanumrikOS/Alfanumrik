...
function Metric({ label, value, detail }: { label: string; value: number | string | null; detail: string }) {
  return <Surface className="p-4"><p className="text-sm text-secondary-ink">{label}</p><p className="mt-1 text-2xl font-bold">{value ?? '—'}</p><p className="mt-2 text-xs text-secondary-ink">{detail}</p></Surface>;
}

export function SchoolV3Overview() {
  const { schoolName, academicYear } = useSchoolV3Scope();
      ...
      {overview.loading ? <DataState state="loading" title="Loading governed school metrics" /> : overview.error || !overview.data ? <DataState state="error" title="School overview is temporarily unavailable" action={<Button onClick={overview.retry}>Try again</Button>} /> : (
        <>
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <Metric label="Active students" value={overview.data.data.active_students} detail="School overview read model" />
            <Metric label="Teachers" value={overview.data.data.teacher_count} detail="Active school roster" />
            <Metric label="Seat use" value={overview.data.data.seat_utilization_pct == null ? null : `${overview.data.data.seat_utilization_pct}%`} detail="Current licensed capacity" />
            <Metric label="Average mastery" value={overview.data.data.avg_mastery == null ? null : `${Math.round(overview.data.data.avg_mastery * 100)}%`} detail="BKT mastery, active roster" />
          </div>
          <Surface className="p-5">
            <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-bold">Classes requiring intervention</h2><p className="mt-1 text-sm text-secondary-ink">Source: governed class-risk read model · refreshed on page load</p></div><Link className="v3-button v3-button--secondary" href={`/school-admin/insights?academicYear=${encodeURIComponent(academicYear)}`}>Open insights</Link></div>
