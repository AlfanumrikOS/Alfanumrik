...
export default function SchoolAdminV3LayoutGate({ children }: { children: React.ReactNode }) {
  ...
  if (!enabled) return <SchoolAdminShell>{children}</SchoolAdminShell>;
  if (!manifest || !routeAllowed) return <DataState state="permission" title="This school destination is unavailable" />;
  return <SchoolAdminV3Shell manifest={manifest}>{children}</SchoolAdminV3Shell>;
}
