...
export default function SchoolAdminV3LayoutGate({ children }: { children: React.ReactNode }) {
  const { enabled, loading, manifest, routeAllowed, scope, legacyAllowed, denied } = useExperienceV3('school-admin');
  ...
  if (denied || !enabled || !manifest || !routeAllowed) return <DataState state="permission" title="This school destination is unavailable" />;
