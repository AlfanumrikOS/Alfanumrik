...
describe('API helpers', () => {
  ...
  it('supabase exports essential functions', async () => {
    const api = await import('@alfanumrik/lib/supabase');
  ...
  it('supabase URL and key are exported', async () => {
    const { supabaseUrl, supabaseAnonKey } = await import('@alfanumrik/lib/supabase');
  ...
  it('chatWithFoxy handles timeout gracefully', async () => {
    const { chatWithFoxy } = await import('@alfanumrik/lib/supabase');
...
describe('Constants', () => {
  it('SUBJECT_META has essential subjects', async () => {
    const { SUBJECT_META } = await import('@alfanumrik/lib/constants');
  ...
  it('GRADES covers 6-12', async () => {
    const { GRADES } = await import('@alfanumrik/lib/constants');
  ...
  it('LANGUAGES includes Hindi and English', async () => {
    const { LANGUAGES } = await import('@alfanumrik/lib/constants');
