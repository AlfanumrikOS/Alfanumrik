 ...
 *
 * compute_gst is mocked at the @alfanumrik/lib/gst seam so there is no DB. The Razorpay
...
vi.mock('@alfanumrik/lib/gst', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@alfanumrik/lib/gst')>();
  return {
    ...actual,
