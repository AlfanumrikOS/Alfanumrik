 ...
 * (src/__tests__/api/teacher/remediation/route.test.ts): @alfanumrik/lib/rbac is stubbed
 * so authorizeRequest / canAccessStudent / hasAnyPermission / logAudit are
 * controllable + observable; @alfanumrik/lib/supabase-admin is a tiny in-memory query
 * builder; and @alfanumrik/lib/pulse/pulse-server is stubbed so we can assert it is NEVER
