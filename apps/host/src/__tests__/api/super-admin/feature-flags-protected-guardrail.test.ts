...
describe('protected_feature_flags DB/TS registry parity', () => {
  ...
  it('ff_foxy_openai_primary_rollout_v1 (added 2026-08-03, REG-334/REG-335, migration 20260803120001) is present in both the TS registry and the DB seed with tier ai_provider, and IS in EXPECTED_OFF_FLAGS again as of 2026-09-01', () => {
    const rows = parseSeededRows();
    const flagName = 'ff_foxy_openai_primary_rollout_v1';
    // Stays CONSOLE-protected at ai_provider (typed confirmation for any
    // further change) in both TS and the DB mirror — that never changed.
    ...
    // Canary-watched again. From 2026-08-03 this asserted the OPPOSITE, while
    // the lever was CEO-approved intentionally-live at enabled=true/100
    // (#1443). Migration 20260901140000 returned it to enabled=false/0 on CEO
    // direction (Anthropic primary for Foxy teaching, OpenAI fallback), so the
    // posture canary must once again treat enabled=true/100 as drift.
    expect(EXPECTED_OFF_FLAGS.includes(flagName), `${flagName} should be in EXPECTED_OFF_FLAGS (lever returned to 0% on 2026-09-01)`).toBe(true);
  });
});
