...
vi.mock('@alfanumrik/lib/cache', async (importOriginal) => {
  const real = await importOriginal<typeof import('@alfanumrik/lib/cache')>();
  return {
    ...real,
...
vi.mock('@alfanumrik/lib/learn/weekly-dive-orchestrator', async (importOriginal) => {
  const real = await importOriginal<typeof import('@alfanumrik/lib/learn/weekly-dive-orchestrator')>();
  return {
    ...real,
...
describe('GET /api/dive/state', () => {
  ...
  it('state is completed when artifact exists for current ISO week', async () => {
    ...
    const { isoWeekOf } = await import('@alfanumrik/lib/learn/weekly-dive-orchestrator');
