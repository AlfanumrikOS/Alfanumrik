 ...
 * business logic):
 *   - `@alfanumrik/lib/supabase-admin` (supabaseAdmin): a recording proxy that captures
 ...
 *     through this client.
 *   - `@alfanumrik/lib/supabase-server` (createSupabaseServerClient): a separate
 *     recording proxy for the RLS-respecting `student_bookmarks` insert on the
 *     `save` path.
 *   - `@alfanumrik/lib/state/events/publish` (publishEvent): captured; asserted to be the
...
vi.mock('@alfanumrik/lib/learn/foxy-expectations', () => ({
