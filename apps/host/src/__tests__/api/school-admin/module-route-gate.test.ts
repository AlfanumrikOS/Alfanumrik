 ...
 *   - `@alfanumrik/lib/school-admin-auth`  → control whether auth passes (and the schoolId).
 *   - `@alfanumrik/lib/modules/route-guard`→ spy on `assertModuleEnabledForSchool` so we can
 ...
 *   - `@alfanumrik/lib/supabase-admin`     → a thin chainable stub so the happy path reaches
...
vi.mock('@alfanumrik/lib/modules/route-guard', () => ({
