...
vi.mock('@alfanumrik/lib/supabase', () => ({ supabase: { from: vi.fn(() => ({ select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), single: vi.fn().mockResolvedValue({ data: null }), then: (r: (v: unknown) => unknown) => r({ data: null }) })) } }));
...
vi.mock('@alfanumrik/lib/use-atlas-flag', () => ({ useAtlasFlag: () => atlasHolder.enabled }));
