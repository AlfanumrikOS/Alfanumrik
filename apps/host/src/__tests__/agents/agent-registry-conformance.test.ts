...
describe('GenAI Phase 3 — Agent Registry conformance (spec §3 a–f)', () => {
  ...
  it('(d) live agents have a real on-disk entryPoint; planned agents have null', () => {
    ...
    // Sanity: the 7 known live agents are exactly the live set. `outcome_prediction`
    ...
    // gated by ff_lesson_generation_v1), taking the live set from 5 → 6; `content_generation`
    // went live 2026-07-24 (GenAI Phase 5c — apps/host/src/app/api/content/diagram/route.ts,
    // gated by ff_content_generation_v1), taking the live set from 6 → 7 (all agents now live).
