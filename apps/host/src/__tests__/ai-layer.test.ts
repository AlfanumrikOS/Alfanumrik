...
vi.mock('@alfanumrik/lib/ai/workflows/explain', () => ({ runExplainWorkflow: vi.fn() }));
vi.mock('@alfanumrik/lib/ai/workflows/doubt-solve', () => ({ runDoubtWorkflow: vi.fn() }));
vi.mock('@alfanumrik/lib/ai/workflows/quiz-generate', () => ({ runQuizGenerateWorkflow: vi.fn() }));
vi.mock('@alfanumrik/lib/ai/workflows/revision', () => ({ runRevisionWorkflow: vi.fn() }));

// ─── Imports (after mocks) ────────────────────────────────────────────────────

import { classifyIntent } from '@alfanumrik/lib/ai/workflows/foxy-router';
import { validateQuizQuestions } from '@alfanumrik/lib/ai/validation/quiz-validator';
import { validateOutput } from '@alfanumrik/lib/ai/validation/output-guard';
import { validateContentScope } from '@alfanumrik/lib/ai/validation/content-guard';
import { getAIConfig } from '@alfanumrik/lib/ai/config';
import { TraceLogger } from '@alfanumrik/lib/ai/tracing/trace-logger';
