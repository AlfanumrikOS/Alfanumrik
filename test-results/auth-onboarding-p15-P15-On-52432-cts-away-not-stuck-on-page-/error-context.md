...
- Name: auth-onboarding-p15.spec.ts >> P15: Onboarding Integrity >> B: Student onboarding happy path >> /onboarding unauthenticated student redirects away (not stuck on page)
- Location: e2e\auth-onboarding-p15.spec.ts:168:9
...
> 180 |       expect(stuckOnOnboarding).toBe(false);
  ...
  259 |   });
  260 | 
  261 |   // ── D. Guardian/parent onboarding redirect ────────────────────────────────
  262 | 
  263 |   test.describe('D: Guardian role redirected from /onboarding to /parent', () => {
  264 | 
