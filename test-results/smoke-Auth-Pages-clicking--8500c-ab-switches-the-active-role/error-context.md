...
- Name: smoke.spec.ts >> Auth Pages >> clicking Teacher tab switches the active role
- Location: e2e\smoke.spec.ts:211:7
...
Call log:
  - waiting for locator('button:has-text("Teacher")')
    - locator resolved to <button role="tab" aria-selected="false" class="flex-1 py-2.5 rounded-xl text-xs font-bold transition-all">…</button>
