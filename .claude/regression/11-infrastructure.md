...
## 2026-08-03 P0+P1 launch-hardening batch — global SWR provider, tenant-lookup fail-open, health-poll de-duplication, coverage-gate honesty — REG-338..REG-341
...
infrastructure pins; the sibling entries from the same batch are REG-334
(`04-payments.md`), REG-335 (`05-xp-scoring.md`), REG-336
(`07-teacher-school.md`) and REG-337 (`10-rbac-rls.md`).

**REG-338 (P0-2).** `DEFAULT_CONFIG` in `packages/lib/src/swr.tsx` (bounded
...
**REG-339 (P0-5).** `getSchoolBySlug` / `getSchoolByCustomDomain` in
...
**REG-340 (P0-4).** `ci.yml`'s post-deploy health-check job DUPLICATED
...
**REG-341 (P1-7).** The coverage gate was measuring the WRONG surface — and
...
- NO dedicated meta-test pins the config shape itself (a test asserting
  ...
  the repaired config plus its embedded rationale, which is why REG-341 is
  recorded as P, not E. Writing that meta-test is the ratchet path to E.

### Invariants covered by this section

- P10-adjacent (REG-338) — retry behaviour on Indian 4G is bounded globally,
  not only in the hooks that happened to live in `swr.tsx`.
- White-label tenant availability / operational integrity (REG-339) — a
...
- Deployment integrity (REG-340) — exactly one health authority polls the
  exact deployed SHA; the duplicate-poller failure mode is mutation-pinned.
- Gate honesty (REG-341, REG-317 family) — a coverage gate that measures
...
Pre-REG-338: 337 entries (through REG-337, the verifyCronAuth consolidation
— see `10-rbac-rls.md`). The 2026-08-03 P0+P1 batch adds REG-338 (global SWR
provider mount), REG-339 (proxy school-lookup fail-open), REG-340 (CI/deploy
health-poll single home) and REG-341 (coverage-gate integrity) — the last
four of the eight ids REG-334..REG-341 consumed by this batch.
**Total catalog: 341 entries (target: 35 — TARGET EXCEEDED).**
