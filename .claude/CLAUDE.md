...
| Non-AI Edge Functions | `supabase/functions/daily-cron/`, `queue-consumer/`, `send-*-email/`, `scan-ocr/`. CORRECTION 2026-08-16 (Phase 0 super-admin audit): `export-report/` does not exist on disk (`ls supabase/functions/` has no such directory) and was removed from this row — do not re-add without verifying it was actually re-created. CORRECTION 2026-09-05 (P2-9): `session-guard/` was undeployed and no longer exists on disk — removed from this row. |
