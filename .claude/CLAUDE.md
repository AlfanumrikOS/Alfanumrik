...
| NCERT ingestion pipeline | `scripts/ncert-ingestion/` (repo root). PDF → `pdf-parse` → chapter-split → ~400-token chunks → `rag_content_chunks` (`source='ncert_2025'`) → Voyage `voyage-3` (1024-d) via `embed-chunks.ts`. Entry points: `discover.ts`, `ingest-local.ts`, `storage-ingest.ts` (Storage bucket `ncert-books`), `validate.ts`, `rollback.ts`. npm scripts live in **`apps/host/package.json`**, not the root: `ncert:discover`, `ncert:ingest`, `ncert:embed`, `ncert:validate`, `ncert:pipeline` (= ingest && validate; does **not** embed). Source PDFs gitignored; they live in Storage bucket `ncert-books`. **`ncert:embed` spends real Voyage API money.** ⚠️ Open question (do not assert either side): `docs/runbooks/ingest-ncert-french-revolution.md:467` says the existing 16,006 chunks came from "a legacy tool no longer present in the codebase" — both can be true (retired builder + successor pipeline). Provenance of the existing chunks is unconfirmed. |
| NCERT corpus — **content EXISTS, do not re-ingest blind** | ~**16,006 chunks** in `rag_content_chunks` covering **750 of 761** `cbse_syllabus` rows (~98.6%), per the 2026-07 audits. The gap is *visibility*, not content. `rag_status='ready'` requires `chunk_count >= 50` AND `verified_question_count >= 40` (`recompute_syllabus_status()`), so a fully-ingested chapter still reads `'partial'` when its questions are merely unverified — **`'partial'` does not mean missing text.** Existing unsurfaced ledger: `ingestion_gaps` view (non-ready in-scope chapters + severity), `cbse_syllabus_rag_diagnostic` view (denormalized `chunk_count` vs actual, `sync_state='STALE'`), `subject_content_readiness_daily` table. |
...
| Migrations | `supabase/migrations/` — **root holds the baseline + 409 timestamped migrations (410 `.sql` files total; latest `20260716120000_seed_ff_foxy_math_format_v2.sql`), verified 2026-07-17.** (Superseded claim, corrected 2026-07-17: the 2026-05-03 Section 10 text said "root contains 0 timestamped migrations until `capture-and-pr` lands the baseline" — the baseline landed and 409 migrations have accumulated on top of it since. Re-count with `ls supabase/migrations/*.sql | wc -l` before quoting a number.) The 349-file pre-baseline chain plus the original 10 pre-timestamp legacy files remain archived under `supabase/migrations/_legacy/` and `_legacy/timestamped/`. **Schema reproducibility P0 fix runbook: `docs/runbooks/schema-reproducibility-fix.md`** — replaces the legacy chain with a pg_dump-derived idempotent baseline (`00000000000000_baseline_from_prod.sql`), pre-marked applied on prod and main-staging via `supabase migration repair` so the merge skips execution on those envs and only runs against fresh projects (CI live-DB tests, new staging, DR). Supabase CLI's `db push` only applies files at the immediate `supabase/migrations/` root, so `_legacy/` is skipped automatically on every deploy. |
