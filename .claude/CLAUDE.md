...
| Support ticket operator console | Served at `/internal/admin` (`SupportTab` in `apps/host/src/app/internal/admin/_components/SupportTab.tsx`), backed by `/api/internal/admin/support`. **`/super-admin/support` (`apps/host/src/app/super-admin/support/page.tsx`) does NOT surface ticket content** — it shows user-activity/diagnostics lookups (quiz/chat sessions, parent links, class mappings), not the actual support queue. Known gap found in the 2026-08-12 F12 audit; full page-wiring fix deferred, not yet scheduled. UPDATE 2026-08-16 (Phase 0 of the super-admin mission-control overhaul, CEO-authorized): Phase 0 adds a real "Support Tickets" nav entry in `/super-admin` plus a cross-link from `/super-admin/support` to `/internal/admin`'s working queue as an interim bridge; the full console merge (support queue first, capability-parity tests before any legacy deletion) is scheduled for Phase 2. See `docs/superpowers/specs/2026-08-16-super-admin-mission-control-design.md` and `docs/runbooks/super-admin-orphaned-apis.md`. |
