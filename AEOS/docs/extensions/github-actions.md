...
- **quality** ("Quality Gate (verify CI green for exact SHA)" — since 2026-07-20 it does NOT re-run lint/type-check/test/build; it is a fail-closed poll of the Actions API, every 30s bounded at 20 min, requiring the "CI — Alfanumrik" workflow's "CI Gate" job to conclude `success` for the exact deploy SHA. Missing run, failure/cancelled/skipped/timed_out conclusion, or timeout all fail the gate and block everything downstream. A "Quality Gate" failure with a *cancelled* CI run usually means the push was superseded — per-ref concurrency cancelled the older CI run, and only the newest green SHA deploys; to genuinely redeploy that SHA, re-run its CI to green first, then re-run the deploy) → **pre-deploy-checklist** (pending-migration list, destructive-change scan for DROP/TRUNCATE, secret-in-diff scan, env summary incl. region `bom1`) → **migrations** (`supabase db push --linked --include-all` to prod, `environment: production`) → **deploy-functions** (deploys only changed Edge Functions; all active if `_shared/` changed) → **health-check** (canonical domain, auto-rollback on real failure, soft-pass on 429 checkpoint) → **post-deploy-verify** (critical endpoints + security-header presence) → **release** (GitHub release tag from `package.json` version).

**`deploy-staging.yml`** — push to `develop`/`staging` (+ manual dispatch). quality → **migrations to staging** (dedicated staging Supabase org token) → **Vercel preview deploy** (comments preview URL on associated PRs) → **deploy-functions (staging)** → **health-check** against the preview URL. The quality job is split by trigger: on push to `develop` ONLY, it uses the same fail-closed "CI Gate" wait as deploy-production (the poll script is intentionally duplicated between the two workflows — keep both in sync); push to `staging` and workflow_dispatch keep the full inline lint/type-check/test/build steps (no ci.yml run exists for those refs).
