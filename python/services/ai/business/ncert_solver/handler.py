...
def detectType(text: str, options: Optional[list[str]] = None, marks: Optional[int] = None) -> str:
    lower = text.lower()
    if options and len(options) >= 3: return 'mcq'
    if re.search(r'assertion.*reason', lower): return 'assertion_reasoning'
    if re.search(r'case.?study|passage|comprehension', lower): return 'case_based'
    if re.search(r'grammar|tense|voice|narration', lower): return 'grammar'
    if re.search(r'poem|stanza|character|novel', lower): return 'literature'
    if re.search(r'calculate|find.*value|solve|simplify|prove', lower): return 'numerical'
    if marks and marks >= 5: return 'long_answer'
    return 'short_answer'

def parseQuestion(text: str, subject: str, grade: str, options: Optional[list[str]] = None, marks: Optional[int] = None) -> ParsedQuestion:
    qtype = detectType(text, options, marks)
    hasNumerical = bool(re.search(r'\d+\s*[\+\-\×\÷\*\/\=]|\bcalculate\b|\bfind.*value\b|\bsolve\b', text, re.IGNORECASE))
    hasFormula = bool(re.search(r'[=><≥≤±√]|x\^|sin|cos|formula', text, re.IGNORECASE))
    effectiveMarks = marks if marks else (1 if qtype == 'mcq' else 2 if qtype == 'short_answer' else 5)
    depth = 'brief' if effectiveMarks <= 1 else 'moderate' if effectiveMarks <= 3 else 'detailed'
    
    return ParsedQuestion(
        originalText=text, type=qtype, subject=subject, grade=grade,
        concepts=[], marks=effectiveMarks, expectedDepth=depth,
        hasNumerical=hasNumerical, hasFormula=hasFormula, options=options or []
    )
...
def routeToSolver(parsed: ParsedQuestion) -> RouteInfo:
    ...
    if qtype == 'mcq':
        return RouteInfo(solver='hybrid' if num else 'retrieval', requiresVerification=True, maxResponseTokens=400)
...
def buildSolverSystemPrompt(parsed: ParsedQuestion, ragContext: Optional[str]) -> str:
    grade = parsed.grade
    ...
    elif subjectLower in ['history', 'geography', 'civics', 'economics', 'social science', 'political science']:
...
def buildSolverPrompt(parsed: ParsedQuestion, route: RouteInfo, ragContext: Optional[str], gradeStyle: str) -> str:
    formatRules = ""
    if parsed.type == 'mcq':
    ...
    marksGuide = '1-2 sentences.' if parsed.marks <= 1 else '3-5 sentences with concept.' if parsed.marks <= 3 else 'Detailed with definition, explanation, example.'
    noRagWarning = "" if ragContext else "\nIMPORTANT: No NCERT reference material was retrieved. Include a note in your explanation that the student should verify this answer from their NCERT textbook."
...
def buildVerificationPrompt(parsed: ParsedQuestion, proposedAnswer: str) -> str:
    opts_str = f"OPTIONS: {' | '.join([f'{chr(65+i)}) {o}' for i, o in enumerate(parsed.options)])}" if parsed.options else ""
    task1 = 'RECOMPUTE all calculations independently from scratch. Check units, significant figures, and sign.' if parsed.hasNumerical else f'Check all key concepts, facts, and definitions against NCERT for Class {parsed.grade}.'
...
def estimateConfidence(solver: str, verified: bool, hasRAG: bool) -> float:
    c = 0.9 if solver == 'deterministic' else 0.8 if solver == 'rule_based' else 0.75 if solver == 'hybrid' else 0.65
    if hasRAG: c += 0.1
    else: c -= 0.15
    if verified: c += 0.05
    else: c -= 0.15
...
async def fetchRAGContext(*args, **kwargs) -> Optional[str]:
...
async def handle_ncert_solver(req: NcertSolverRequest, auth_header: str) -> NcertSolverResponse:
    ...
    async with httpx.AsyncClient() as hc:
        ...
        res = await hc.get(auth_url, headers={
    ...
    try:
        student_res = await client.table("students").select("id").eq("auth_user_id", user_id).eq("is_active", True).is_("deleted_at", "null").execute()
        student_data = student_res.data
        if not student_data:
            raise HTTPException(status_code=422, detail={"error": "subject_not_allowed", "reason": "grade", "subject": req.subject})
        resolved_student_id = student_data[0]["id"]
        
        subj_check = await client.rpc("get_available_subjects", {"p_student_id": resolved_student_id}).execute()
        subj_rows = subj_check.data or []
        row = next((r for r in subj_rows if r.get("code") == req.subject), None)
        if not row:
            raise HTTPException(status_code=422, detail={"error": "subject_not_allowed", "reason": "grade", "subject": req.subject})
        if row.get("is_locked"):
            raise HTTPException(status_code=422, detail={"error": "subject_not_allowed", "reason": "plan", "subject": req.subject})

        usage_date = time.strftime("%Y-%m-%d")
        usage_res = await client.rpc("check_and_record_usage", {
    ...
    solution = {"answer": solutionRaw, "steps": [], "concept": "", "explanation": solutionRaw, "common_mistake": "", "formula_used": ""}
    try:
