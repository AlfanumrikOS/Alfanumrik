...
from .models import ResponseSoFar, QuizGeneratorRequest, QuizGeneratorResponse, QuizGeneratorMeta, QuestionRow
from python.services.ai.shared.quiz_oracle import run_deterministic_checks, CandidateQuestion
...
async def resolve_subject_id(supabase, subject_code: str) -> Optional[str]:
    res = supabase.table('subjects').select('id').eq('code', subject_code).eq('is_active', True).maybe_single().execute()
    if res.data:
...
async def check_rate_limit_db(student_id: str, supabase) -> bool:
    ...
    res = supabase.table('quiz_sessions').select('*', count='exact', head=True).eq('student_id', student_id).gte('created_at', window_start).execute()
    count = res.count if res.count is not None else 0
    return count < 20

async def fetch_due_review_questions(supabase, student_id: str, subject_id: str, subject_code: str, grade: str, max_count: int, exclude_ids: Set[str]) -> Tuple[List[Dict[str, Any]], int]:
    if max_count <= 0:
        return [], 0
    today_str = datetime.datetime.utcnow().strftime('%Y-%m-%d')
    due_res = supabase.table('spaced_repetition_cards').select('source_id, topic, ease_factor, next_review_date') \
        .eq('student_id', student_id).eq('subject', subject_code).eq('is_active', True) \
        .lte('next_review_date', today_str).order('next_review_date', desc=False).order('ease_factor', desc=False).execute()
    
    due_cards = due_res.data or []
    if not due_cards:
        return [], 0
        
    candidate_ids = [c['source_id'] for c in due_cards if c.get('source_id') and c['source_id'] not in exclude_ids]
    if not candidate_ids:
        return [], 0
        
    qs_res = supabase.table('question_bank').select('*').in_('id', candidate_ids).eq('is_active', True).execute()
...
async def is_irt_selection_enabled(supabase) -> bool:
    res = supabase.table('feature_flags').select('is_enabled, rollout_percentage').eq('flag_name', 'ff_irt_question_selection').maybe_single().execute()
    if res.data and res.data.get('is_enabled') and res.data.get('rollout_percentage', 0) >= 100:
        return True
    return False

async def select_questions_by_irt(supabase, student_id: str, subject: str, grade: str, chapter_number: Optional[int], count: int, exclude_ids: Set[str]) -> List[Dict[str, Any]]:
    exclude = list(exclude_ids)
    res = supabase.rpc('select_questions_by_irt_info', {
...
async def select_adaptive_questions(supabase, student_id: str, subject_id: str, subject_code: str, grade: str, count: int, exclude_ids: Set[str]) -> Tuple[List[Dict[str, Any]], int]:
    now = datetime.datetime.utcnow().isoformat()
    mastery_res = supabase.table('concept_mastery').select(
        'topic_id, mastery_level, next_review_at, curriculum_topics!inner(subject_id, chapter_number, concept_tag)'
    ).eq('student_id', student_id).eq('curriculum_topics.subject_id', subject_id).lt('mastery_level', 0.95).order('mastery_level', desc=False).limit(20).execute()
    ...
    for topic in target_topics:
        ...
        query = supabase.table('question_bank').select('*').eq('subject', subject_code).eq('grade', grade).eq('is_active', True)
        if exclusion_list:
        ...
        if len(qs) < need and concept_tag:
            q2 = supabase.table('question_bank').select('*').eq('subject', subject_code).eq('grade', grade).eq('is_active', True)
            if exclusion_list: q2 = q2.not_.in_('id', exclusion_list)
            if chapter_num is not None: q2 = q2.eq('chapter_number', chapter_num)
            q2 = q2.eq('difficulty', target_diff).in_('bloom_level', allowed_blooms)
            res2 = q2.limit(need * 2).execute()
            qs = res2.data or qs
            
        if len(qs) < need:
            q3 = supabase.table('question_bank').select('*').eq('subject', subject_code).eq('grade', grade).eq('is_active', True)
            if exclusion_list: q3 = q3.not_.in_('id', exclusion_list)
            if chapter_num is not None: q3 = q3.eq('chapter_number', chapter_num)
...
async def select_random_questions(supabase, subject_code: str, grade: str, count: int, difficulty: Optional[int], exclude_ids: Set[str], chapter_number: Optional[int] = None) -> List[Dict[str, Any]]:
    query = supabase.table('question_bank').select('*').eq('subject', subject_code).eq('is_active', True).eq('grade', grade)
...
async def fetch_seen_question_ids(supabase, student_id: str, subject: str, grade: str, chapter_number: Optional[int]) -> Set[str]:
    query = supabase.table('user_question_history').select('question_id').eq('student_id', student_id).eq('subject', subject).eq('grade', grade)
    if chapter_number is not None:
        query = query.eq('chapter_number', chapter_number)
    res = query.limit(500).execute()
    return set(r['question_id'] for r in (res.data or []))

async def check_and_reset_history(supabase, student_id: str, subject: str, grade: str, chapter_number: Optional[int], total_pool: int):
    query = supabase.table('user_question_history').select('*', count='exact', head=True).eq('student_id', student_id).eq('subject', subject).eq('grade', grade)
    if chapter_number is not None:
        query = query.eq('chapter_number', chapter_number)
    res = query.execute()
    seen_count = res.count if res.count is not None else 0
    if total_pool > 0 and (seen_count / total_pool) >= 0.8:
        del_query = supabase.table('user_question_history').delete().eq('student_id', student_id).eq('subject', subject).eq('grade', grade)
        if chapter_number is not None:
            del_query = del_query.eq('chapter_number', chapter_number)
        del_query.execute()

async def record_shown_questions(supabase, student_id: str, subject: str, grade: str, questions: List[Dict[str, Any]]):
    ...
    supabase.table('user_question_history').upsert(rows, on_conflict='student_id,question_id').execute()
...
def compute_adaptive_difficulty(responses: List[ResponseSoFar]) -> Dict[str, Any]:
    ...
    avg_correct_time = sum(r.time_spent for r in recent_correct) / len(recent_correct) if recent_correct else 15
...
async def handle_next_question(supabase, body: QuizGeneratorRequest) -> QuizGeneratorResponse:
    ...
    query = supabase.table('question_bank').select('*').eq('subject', subject).eq('grade', grade).eq('is_active', True)
    if exclusion_list: query = query.not_.in_('id', exclusion_list)
    if chapter_number is not None: query = query.eq('chapter_number', chapter_number)
    ...
    if not candidates:
        q2 = supabase.table('question_bank').select('*').eq('subject', subject).eq('grade', grade).eq('is_active', True)
        if exclusion_list: q2 = q2.not_.in_('id', exclusion_list)
        if chapter_number is not None: q2 = q2.eq('chapter_number', chapter_number)
    ...
    if not candidates:
        ...
        q3 = supabase.table('question_bank').select('*').eq('subject', subject).eq('grade', grade).eq('is_active', True)
        if exclusion_list: q3 = q3.not_.in_('id', exclusion_list)
        if chapter_number is not None: q3 = q3.eq('chapter_number', chapter_number)
...
async def generate_quiz(supabase, body: QuizGeneratorRequest) -> QuizGeneratorResponse:
    ...
    if difficulty is None and ability_estimate is not None:
        if ability_estimate < -1.0: difficulty = 1
        elif ability_estimate < 0.5: difficulty = 2
        else: difficulty = 3
    ...
    pool_query = supabase.table('question_bank').select('*', count='exact', head=True).eq('subject', subject).eq('grade', grade).eq('is_active', True)
    if chapter_number is not None:
    ...
    if body.difficulty is None:
        ...
        review_questions, review_topic_count = await fetch_due_review_questions(supabase, student_id, subject_id, subject, grade, review_slots, seen_ids)
    ...
    if body.difficulty is None and adaptive_slots > 0:
        ...
        if use_irt:
            irt_qs = await select_questions_by_irt(supabase, student_id, subject, grade, chapter_number, adaptive_slots, used_after_review)
            if len(irt_qs) >= adaptive_slots:
                adaptive_questions = irt_qs
                weak_topics_targeted = 0
                
        if not adaptive_questions:
            ad_qs, wt_count = await select_adaptive_questions(supabase, student_id, subject_id, subject, grade, adaptive_slots, used_after_review)
    ...
    if len(questions) < count:
        ...
        random_qs = await select_random_questions(supabase, subject, grade, remaining, difficulty, used_ids, chapter_number)
        questions += random_qs
