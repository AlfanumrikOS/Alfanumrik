...
from .models import (
    ...
    ConceptStateRequest, ConceptStateListResponse, ConceptStateItem,
    RevisionScheduleResponse, ExamReadinessRequest, ExamReadinessResponse
)
from .handler import (
    updateMastery, selectNextAction, computeRevisionSchedule, computeExamReadiness, computeRetention
)
...
async def get_current_student(authorization: str = Header(None)) -> Dict[str, Any]:
    ...
    res = await client.table("students").select("id, grade, preferred_subject").eq("auth_user_id", user_id).eq("is_active", True).execute()
    if not res.data:
...
async def next_action(req: NextActionRequest, student: Dict[str, Any] = Depends(get_current_student)):
    client = get_service_client()
    ...
    states_res = await client.table("cme_concept_state").select("*").eq("student_id", student["id"]).execute()
    topics_res = await client.table("curriculum_topics").select("id,title,parent_topic_id,prerequisite_topic_ids,difficulty_level,bloom_focus,chapter_number,display_order,grade,subject_id").eq("is_active", True).is_("deleted_at", "null").execute()
    ...
    # Log action
    await client.table("cme_action_log").insert({
...
async def record_response(req: RecordResponseRequest, student: Dict[str, Any] = Depends(get_current_student)):
    client = get_service_client()
    
    existing_res = await client.table("cme_concept_state").select("*").eq("student_id", student["id"]).eq("concept_id", req.concept_id).execute()
    
    current_state = existing_res.data[0] if existing_res.data else {
        "mastery_mean": 0.3,
    ...
    await client.table("cme_concept_state").upsert(upsert_data, on_conflict="student_id,concept_id").execute()

    if not req.correct and updated.get("errorType"):
        await client.table("cme_error_log").insert({
            "student_id": student["id"],
            "concept_id": req.concept_id,
...
async def get_concept_state(req: ConceptStateRequest, student: Dict[str, Any] = Depends(get_current_student)):
    client = get_service_client()
    ...
    states_res = await client.table("cme_concept_state").select(
    ...
    states = states_res.data or []
    for s in states:
        s["current_retention"] = computeRetention(s["mastery_mean"], s["retention_half_life"], s.get("last_practiced_at"))
...
async def get_revision_due(student: Dict[str, Any] = Depends(get_current_student)):
    ...
    if schedule:
        ...
        topics_res = await client.table("curriculum_topics").select("id, title").in_("id", concept_ids).execute()
...
async def get_exam_readiness(req: ExamReadinessRequest, student: Dict[str, Any] = Depends(get_current_student)):
    client = get_service_client()
    ...
    topics_res = await client.table("curriculum_topics").select(
    ...
    await client.table("cme_exam_readiness").insert({
        "student_id": student["id"],
