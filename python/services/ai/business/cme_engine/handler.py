...
def computeRetention(masteryMean: float, halfLifeHours: float, lastPracticedAt: Optional[str]) -> float:
    ...
    hoursSince = (datetime.now(timezone.utc) - last_dt).total_seconds() / 3600.0
...
def updateMastery(
    ...
    return {
        ...
        "error_count_conceptual": state.get("error_count_conceptual", 0) + (1 if errorType == 'conceptual' else 0),
        "error_count_procedural": state.get("error_count_procedural", 0) + (1 if errorType == 'procedural' else 0),
        "error_count_careless": state.get("error_count_careless", 0) + (1 if errorType == 'careless' else 0),
...
def selectNextAction(
    ...
    relevantTopics = [t for t in topics if t.get("subject_id") == subjectId and t.get("grade") == grade]
    ...
    for topic in relevantTopics:
        ...
        for prereqId in prereqs:
            ...
            if prereqState:
                retention = computeRetention(prereqState["mastery_mean"], prereqState["retention_half_life"], prereqState.get("last_practiced_at"))
                if retention < 0.4:
    ...
    atRisk = []
    for s in states:
        retention = computeRetention(s["mastery_mean"], s["retention_half_life"], s.get("last_practiced_at"))
    ...
    for topic in relevantTopics:
        ...
        if state["mastery_mean"] < 0.6:
            return {
                ...
                "difficulty": state.get("max_difficulty_succeeded") or topic.get("difficulty_level") or 2,
            }
...
def computeRevisionSchedule(states: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for s in states:
        ...
        if retention < 0.7:
            ...
            due_dt = datetime.fromtimestamp(due_ts, timezone.utc)
...
def computeExamReadiness(
    ...
    if not relevant:
        return {"overall": 0, "predicted_percentage": 0, "chapters": {}, "weakest": [], "total_concepts": 0, "concepts_mastered": 0}
    ...
    for topic in relevant:
        ...
        state = stateMap.get(topic["id"])
        if state:
            retention = computeRetention(state["mastery_mean"], state["retention_half_life"], state.get("last_practiced_at"))
    ...
    weakest_list = sorted([{"chapter": k, "score": v} for k, v in chapterScores.items()], key=lambda x: x["score"])
    
    concepts_mastered = 0
    for s in states:
        r = computeRetention(s["mastery_mean"], s["retention_half_life"], s.get("last_practiced_at"))
        if r >= 0.7:
            concepts_mastered += 1
