...
class ShadowPairRow:
    request_id: str
    task_type: str
    shadow_of_request_id: Optional[str]
    grade: Optional[str]
...
async def grade_mol_shadow_pairs(
    ...
    cost_cap_inr: Optional[float] = None,
    grader_cap_inr: Optional[float] = None,
    batch_concurrency: Optional[int] = None,
    estimated_grader_inr_per_call: Optional[float] = None,
) -> GraderCronResult:
    
    if now_fn is None:
        def default_now(): return datetime.now(UTC)
        now_fn = default_now
    ...
    est_inr = estimated_grader_inr_per_call if estimated_grader_inr_per_call is not None else ESTIMATED_GRADER_INR_PER_CALL
    ...
    try:
        res = await client.table("mol_request_logs").select("inr_cost").eq("shadow_role", "shadow").gte("created_at", today_start_iso).execute()
        ...
        if isinstance(data, list):
            ...
            if total_sum > cost_cap:
                ...
                if result.killed:
                    await _emit_kill_switch_audit(client, {
    ...
    try:
        res = await client.table("mol_request_logs")\
    ...
    for i in range(0, len(sampled), concurrency):
        if result.estimated_grader_cost_inr >= grader_cap:
            ...
            logger.warning(f"grader_cron: grader Sonnet cap reached: estimated={result.estimated_grader_cost_inr} cap={grader_cap} — aborting remaining batches")
            result.skipped_no_text += len(sampled) - i
            break

        batch = sampled[i:i + concurrency]
        tasks = [
            _grade_one_pair(client, pair, grader, today)
            for pair in batch
        ]
        ...
        for outcome in settled:
            if isinstance(outcome, Exception):
                ...
                result.estimated_grader_cost_inr = round(result.estimated_grader_cost_inr + est_inr, 4)
                continue
...
async def _grade_one_pair(client: Any, pair: ShadowPairRow, grader: Any, now: datetime) -> Dict[str, Any]:
    ...
    out: Optional[GraderResult] = None
    
    try:
        out = await grader(GraderInput(
...
async def _resolve_texts(client: Any, pair: ShadowPairRow) -> Optional[Dict[str, str]]:
    try:
        res = await client.table("mol_shadow_text_buffer")\
            .select("question_text,baseline_system_prompt,baseline_response_text,shadow_response_text")\
...
async def _cleanup_graded_text(client: Any, shadow_request_id: str) -> None:
    try:
        await client.table("mol_shadow_text_buffer").delete().eq("shadow_request_id", shadow_request_id).execute()
    except Exception as err:
...
async def _flip_kill_switch(client: Any, now: datetime) -> bool:
    try:
        res = await client.table("feature_flags").select("metadata").eq("flag_name", "ff_grounded_answer_mol_shadow_v1").execute()
        data = getattr(res, "data", None)
        if data is None and isinstance(res, dict):
            data = res.get("data")
...
async def _write_grader_telemetry(client: Any, pair: ShadowPairRow, result: Optional[GraderResult], latency_ms: int) -> None:
    try:
        failed = result is None
        await client.table("mol_request_logs").insert({
