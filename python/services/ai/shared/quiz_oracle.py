...
class LlmGradeResult(BaseModel):
    ...
    reasoning: str
    suggested_correct_index: Literal[0, 1, 2, 3] | None = None
...
def check_numeric_consistency(
    question_text: str, correct_option_text: str, explanation: str
) -> str | None:
...
def run_deterministic_checks(q: CandidateQuestion) -> OracleRejectResult | None:
    ...
    if PLACEHOLDER_RE.search(text):
        return reject_det(
            "p6_text_empty_or_placeholder", "question_text contains {{ or [BLANK] placeholder"
        )

    if not isinstance(q.options, list) or len(q.options) != 4:
        return reject_det(
            "p6_options_not_4",
            f"expected exactly 4 options, got {len(q.options) if isinstance(q.options, list) else 'non-array'}",
        )
    ...
    if len(distinct) != 4:
        return reject_det(
            "p6_options_not_distinct", "options are not all distinct (case-insensitive)"
        )

    idx = q.correct_answer_index
    if not isinstance(idx, int) or idx < 0 or idx > 3:
        return reject_det(
            "p6_correct_index_out_of_range", f"correct_answer_index must be integer 0..3, got {idx}"
        )
    ...
    for i in range(4):
        for j in range(i + 1, 4):
            ...
            if (a_tokens <= 6 and b_tokens <= 6 and overlap >= 0.7) or overlap >= 0.85:
                return reject_det(
                    "options_overlap_semantic",
                    f'options {i} and {j} overlap (Jaccard={overlap:.2f}): "{a}" vs "{b}"',
                )
