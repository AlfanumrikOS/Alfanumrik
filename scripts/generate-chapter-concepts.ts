 ...
 * (`rag_content_chunks`) using ACTUAL Claude API calls.
 *
 * This is NOT a simulation. Every card is produced by a real Claude call,
 ...
 *
 * MODEL / PROVIDER: reuses the platform's established Claude config
 * (src/lib/ai/config.ts — Haiku primary, Sonnet fallback, same API version /
 * base URL). NO model-family or provider switch (that needs separate user
 * approval). The script calls the Anthropic Messages API directly rather than
 * importing callClaude(), because callClaude() is wired to the Next.js runtime
 * (logOpsEvent writes ops rows, logger) which is inappropriate for a long
 * batch job. The request shape, model names, version, base URL, circuit-breaker
 * spirit, retry/backoff, and safety rails are all preserved.
 *
 * USAGE:
...
// ─── Platform config reuse (model names, version, base URL) ──────────────────
// Mirror src/lib/ai/config.ts exactly — same model family, no provider switch.
const PRIMARY_MODEL = 'claude-haiku-4-5-20251001';
// NOTE: src/lib/ai/config.ts pins the fallback to 'claude-sonnet-4-20250514',
// but that snapshot is NOT available to this API key (404 not_found). The
// available Sonnet in the SAME Claude family is 'claude-sonnet-4-5-20250929'.
// This is a within-family name correction (no provider/family switch), used
// only as the batch fallback. Flagged to ops/architect to reconcile config.ts.
...
// Haiku/Sonnet $ per million tokens (approx, for cost reporting only).
const COST_PER_MTOK: Record<string, { in: number; out: number }> = {
  [PRIMARY_MODEL]: { in: 1.0, out: 5.0 },
  [FALLBACK_MODEL]: { in: 3.0, out: 15.0 },
};
...
async function existingDeckPasses(grade: string, subject: string, chapterNumber: number): Promise<boolean> {
  const r = await rest(
    `chapter_concepts?grade=eq.${grade}&subject=eq.${subject}&chapter_number=eq.${chapterNumber}&source=eq.${NEW_SOURCE}&is_active=is.true` +
      `&select=concept_number,title,explanation,explanation_hi,title_hi,difficulty,bloom_level,practice_question,practice_options`,
  );
  ...
  // Re-run the rubric (chapter-level) against the live rows.
  const cards: GeneratedCard[] = rows.map((x) => ({
    title: x.title ?? '', title_hi: x.title_hi ?? '', learning_objective: '', learning_objective_hi: '',
    ...
    practice_correct_index: 0, practice_explanation: 'x', difficulty: x.difficulty ?? 2, bloom_level: x.bloom_level ?? 'understand',
    estimated_minutes: 5,
  }));
...
// ─── Claude call (direct API, retry/backoff, model fallback) ─────────────────
...
async function callClaudeDirect(
  ...
  try {
    ...
    return { ok: true, text, inputTokens: data.usage?.input_tokens ?? 0, outputTokens: data.usage?.output_tokens ?? 0, stopReason: data.stop_reason ?? null };
  } catch (err) {
    clearTimeout(timer);
...
/** Call Claude with retry/backoff on 429/5xx and Haiku→Sonnet fallback. */
async function generateCards(
  ...
  for (const model of models) {
    for (let attempt = 0; attempt < 3; attempt++) {
      // OUTPUT BUDGET — root cause of the dense-subject failure.
      ...
      // subjects (social_studies, and grade 11/12) overflowed the previous
      // 8000-token ceiling: the model hit stop_reason=max_tokens mid-JSON, the
      // JSON truncated, parse returned 0 cards, and the chapter ended empty
      // (observed: social_studies g7 — stop=max_tokens, len~19763).
      //
      // Claude Haiku 4.5 (claude-haiku-4-5-20251001) supports up to 64,000
      // output tokens, so 8000 was far below the ceiling. We raise to 16000:
      // empirically a full 6-card bilingual deck lands well under this with
      // comfortable headroom, while staying ~4x below Haiku's hard limit.
      // (Model/provider unchanged — Haiku primary, per platform standard.)
      const MAX_OUTPUT_TOKENS = 16_000; // safe ceiling for Haiku 4.5 (limit 64k)
      const r = await callClaudeDirect(model, systemPrompt, userMessage, MAX_OUTPUT_TOKENS);
      if (!r.ok) {
...
async function main(): Promise<void> {
  ...
  const costHaiku = (totalIn / 1e6) * COST_PER_MTOK[PRIMARY_MODEL].in + (totalOut / 1e6) * COST_PER_MTOK[PRIMARY_MODEL].out;
  ...
  console.error(`Tokens:  input=${totalIn}  output=${totalOut}  (~$${costHaiku.toFixed(3)} at Haiku rates)`);
  if (failedVal.length) {
