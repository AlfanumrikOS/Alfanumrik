...
export default function LearnPage() {
  ...
  return(
    <div className="min-h-screen pb-8">
      <div className="sticky top-0 z-50 glass border-b border-white/5"><div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3"><button onClick={()=>router.push('/dashboard')}><ArrowLeft className="w-5 h-5 text-white/40" /></button><div className="flex items-center gap-2"><span className="text-2xl">{getSubjectIcon(subj)}</span><div><div className="font-bold">{isHi?getSubjectLabelHi(subj):getSubjectLabel(subj)}</div><div className="text-xs text-white/25">{concepts.length} {isHi?'अवधारणाएँ':'concepts'}</div></div></div></div></div>
      <div className="max-w-2xl mx-auto px-4 pt-6 space-y-6">
        {/* Progress */}
        <div className="glass rounded-xl p-5"><div className="flex items-center justify-between mb-3"><span className="text-sm font-bold">{isHi?'समग्र प्रगति':'Overall Progress'}</span><span className="text-xs" style={{color}}>0/{concepts.length}</span></div><div className="w-full bg-surface-800/50 rounded-full h-3"><div className="h-3 rounded-full" style={{width:'0%',background:color}} /></div></div>
        {/* Quick Actions */}
        <div className="grid grid-cols-3 gap-3">
          <button onClick={()=>router.push('/foxy')} className="glass rounded-xl p-3 text-center card-interactive"><BookOpen className="w-5 h-5 mx-auto mb-1" style={{color}} /><div className="text-xs font-bold">{isHi?'सीखो':'Learn'}</div></button>
          <button onClick={()=>router.push('/quiz')} className="glass rounded-xl p-3 text-center card-interactive"><Gamepad2 className="w-5 h-5 mx-auto mb-1" style={{color}} /><div className="text-xs font-bold">{isHi?'अभ्यास':'Practice'}</div></button>
          <button onClick={()=>router.push('/quiz')} className="glass rounded-xl p-3 text-center card-interactive"><Brain className="w-5 h-5 mx-auto mb-1" style={{color}} /><div className="text-xs font-bold">{isHi?'क्विज़':'Quiz'}</div></button>
        </div>
        {/* Concept Map */}
        {grades.map(grade=>(
          <div key={grade} className="animate-slide-up">
            <h3 className="text-sm font-bold text-white/40 mb-3 flex items-center gap-2"><span className="w-6 h-6 rounded-md flex items-center justify-center text-xs font-bold" style={{background:`${color}20`,color}}>{grade}</span>{isHi?`कक्षा ${grade}`:`Class ${grade}`}</h3>
            <div className="space-y-2 ml-3 border-l-2 pl-4" style={{borderColor:`${color}20`}}>
              {(byGrade.get(grade)||[]).map(concept=>{
                const isLocked = concept.prerequisites.length > 0; // Simplified — all locked until mastery system kicks in
                return(
