...
{/* Header with animated avatar */}{chapterCtx&&<div style={{padding:'8px 16px',background:'linear-gradient(135deg,#FFF7ED,#FEF3C7)',borderBottom:'1px solid #FDE68A',display:'flex',alignItems:'center',justifyContent:'space-between'}}><div style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:16}}>📖</span><span style={{fontSize:12,fontWeight:700,color:'#92400E'}}>Chapter {chapterCtx.chapter}: {chapterCtx.title}</span></div><button onClick={()=>{setChapterCtx(null);snd('click')}} style={{fontSize:10,color:'#A8A29E',background:'none',border:'none',cursor:'pointer',fontFamily:'inherit',fontWeight:600}}>✕ Exit</button></div>}<div className="a-chat-hdr" style={{gap:10}}>
