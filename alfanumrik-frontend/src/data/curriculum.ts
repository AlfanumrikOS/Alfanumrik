...
// === HELPERS ===
export function getConceptsBySubject(subject: string) { return ALL_CONCEPTS.filter(c => c.subject === subject); }
export function getSubjectIcon(s: string): string { return ({math:'🧮',science:'🔬',english:'📚',hindi:'📝',social_science:'🌍'} as Record<string,string>)[s] ?? '📚'; }
export function getSubjectColor(s: string): string { return ({math:'#FF6B35',science:'#00B4D8',english:'#FFB800',hindi:'#2DC653',social_science:'#9B4DAE'} as Record<string,string>)[s] ?? 'var(--orange)'; }
export function getSubjectLabel(s: string): string { return ({math:'Mathematics',science:'Science',english:'English',hindi:'Hindi',social_science:'Social Science'} as Record<string,string>)[s] ?? s; }
export function getSubjectLabelHi(s: string): string { return ({math:'गणित',science:'विज्ञान',english:'अंग्रेज़ी',hindi:'हिन्दी',social_science:'सामाजिक विज्ञान'} as Record<string,string>)[s] ?? s; }
